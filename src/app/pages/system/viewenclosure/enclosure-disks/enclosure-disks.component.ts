...
export class EnclosureDisksComponent
  ...
  get expanders() {
    if (
      !this.system.platform.includes("MINI") &&
  ...
  constructor(
    ...
    core
      .register({ observerClass: this, eventName: "DiskTemperatures" })
      .subscribe((evt: CoreEvent) => {
        if (
          !this.chassis ||
          !this.chassis[this.view] ||
          !this.chassis[this.view].driveTrayObjects
        ) {
          return;
        }
    ...
    core
      .register({ observerClass: this, eventName: "DisksChanged" })
      .subscribe((evt: CoreEvent) => {
    ...
    this.enclosure.events.subscribe((evt) => {
      switch (evt.name) {
        case "Ready":
          ...
          this.enclosure.container.x =
            this.app._options.width / 2 - this.enclosure.container.width / 2;
          this.enclosure.container.y =
            this.app._options.height / 2 - this.enclosure.container.height / 2;
      ...
      default:
        this.controllerEvents.next({
          ...
          data: {
            name: "Unsupported Hardware",
            message:
              "This chassis has an unknown or missing model value. (METHOD: createExtractedEnclosure)",
          },
        });
        this.aborted = true;
    }
  ...
  extractEnclosure(enclosure, profile) {
    ...
    this.controllerEvents.next({
      ...
      data: { canvas: canvas, profile: profile },
      sender: this,
    });
  ...
  update(className: string) {
    // stage-left or stage-right or expanders

    let sideStage = this.overview.nativeElement.querySelector(
      "." + this.currentView + "." + className
    );
    let html = this.overview.nativeElement.querySelector(
      "." + this.currentView + "." + className + " .content"
    );
    let el = styler(html, {});
  ...
  setDiskHealthState(
      ...
      switch (disk.status) {
        case "ONLINE":
          enclosure.events.next({
            name: "ChangeDriveTrayColor",
            data: { id: disk.enclosure.slot, color: this.theme.green },
          });
          break;
        case "FAULT":
          failed = true;
        ...
        case "AVAILABLE":
          enclosure.events.next({
            name: "ChangeDriveTrayColor",
            data: { id: disk.enclosure.slot, color: "#999999" },
          });
          break;
        default:
          enclosure.events.next({
            name: "ChangeDriveTrayColor",
            data: { id: disk.enclosure.slot, color: this.theme.yellow },
          });
          break;
      }
    }
    ...
    const elements =
      this.system.rearIndex && disk.enclosure.number == this.system.rearIndex
    ...
    if (failed) {
      enclosure.events.next({
        name: "ChangeDriveTrayColor",
        data: { id: disk.enclosure.slot, color: this.theme.red },
      });
    }
  }
      ...
      selectedEnclosure.disks.forEach((disk, index) => {
        ...
        this.enclosure.events.next({
          ...
          data: {
            ...
            color: this.theme[this.theme.accentColors[pIndex]],
          },
        });
      });
    } else {
  ...
  toggleHighlightMode(mode: string) {
    if (this.selectedDisk.status == "AVAILABLE") {
      return;
    }
  ...
  radiate(kill?: boolean) {
    ...
    if (this.identifyBtnRef) {
      ...
      this.identifyBtnRef = {
        animation: animation,
        originalState: startShadow,
        styler: elementBorder,
      };
    }
  }
