...
export class WidgetSysInfoComponent
  ...
  ngAfterViewInit() {
    ...
    this.ws.call("update.get_auto_download").subscribe((res) => {
      if (res == true) {
        this.core
          .register({ observerClass: this, eventName: "UpdateChecked" })
          .subscribe((evt: CoreEvent) => {
  ...
  processSysInfo(evt: CoreEvent) {
    ...
    // PLATFORM INFO
    if (
      this.data.system_manufacturer &&
      this.data.system_manufacturer.toLowerCase() == "ixsystems"
    ) {
  ...
  parseUptime() {
    ...
    if (days > 0) {
      ...
      min === 1
        ? (this.uptimeString += min + T(" minute"))
