...
export class DeviceAddComponent implements OnInit, OnDestroy {
  ...
  preInit() {
    ...
    this.ws.call("vm.resolution_choices").subscribe((res) => {
      const resolution = _.find(this.displayFieldConfig, {
        name: "resolution",
      });
      for (let key in res) {
    ...
    this.core
      .register({ observerClass: this, eventName: "zvolCreated" })
      .subscribe((evt: CoreEvent) => {
        const newZvol = {
          label: evt.data.id,
          value: "/dev/zvol/" + evt.data.id,
        };
        const pathField = _.find(this.diskFieldConfig, { name: "path" });
        pathField.options.splice(
          pathField.options.findIndex((o) => o.value === "new"),
          0,
          newZvol
        );
  ...
  ngOnInit() {
    ...
    this.addZvolComponent = new ZvolWizardComponent(
      this.core,
      this.router,
      this.aroute,
      this.rest,
  ...
  onSubmit(event: Event) {
    ...
    this.aroute.params.subscribe((params) => {
      ...
      deviceValue["physical_sectorsize"] =
        deviceValue["sectorsize"] === 0 ? null : deviceValue["sectorsize"];
      deviceValue["logical_sectorsize"] =
        deviceValue["sectorsize"] === 0 ? null : deviceValue["sectorsize"];
  ...
  updateZvolSearchOptions(value = "", parent) {
    parent.ws
      ...
      .subscribe((zvols) => {
        ...
        _.find(parent.diskFieldConfig, {
          name: "path",
        }).searchOptions = searchedZvols;
      });
  }
