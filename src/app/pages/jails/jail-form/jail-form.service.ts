    ...
    for (const ipType of ["ip4", "ip6"]) {
      ...
      for (let i = 0; i < formGroup.controls[targetPropName].controls.length; i++) {
        ...
        if (addVnet != undefined) {
          subipInterfaceField.options = addVnet
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (formGroup.controls["nat_forwards"].controls[i] == undefined) {
        ...
        const templateListField = _.cloneDeep(
          _.find(networkfieldConfig, { name: "nat_forwards" }).templateListField
        );
  ...
  parseNatForwards(value) {
    if (value["nat_forwards_checkbox"] === true) {
      ...
      for (let i = 0; i < value["nat_forwards"].length; i++) {
        ...
        if (
          Object.values(subNatForward).every(
            (item) => item !== undefined && String(item).trim() !== ""
          )
        ) {
          const length = Object.keys(subNatForward).length;
          if (length === 3) {
            multi_nat_forwards.push(
              subNatForward["protocol"] +
                "(" +
                subNatForward["jail_port"] +
                ":" +
                subNatForward["host_port"] +
                ")"
            );
          } else if (length === 2) {
            multi_nat_forwards.push(
              subNatForward["protocol"] + "(" + subNatForward["jail_port"] + ")"
            );
          }
        }
      }
  ...
  deparseIpaddr(value, ipType, formGroup, basicfieldConfig) {
    ...
    for (let i = 0; i < value.length; i++) {
      if (formGroup.controls[propName].controls[i] == undefined) {
        // add controls;
        const templateListField = _.cloneDeep(
          _.find(basicfieldConfig, { name: propName }).templateListField
        );
      ...
      if (value[i].indexOf("/") > 0) {
        formGroup.controls[propName].controls[i].controls[propName].setValue(
          value[i].split("/")[0]
        );
        formGroup.controls[propName].controls[i].controls[
          ipType + (ipType == "ip4" ? "_netmask" : "_prefix")
        ].setValue(value[i].split("/")[1]);
      } else {
        formGroup.controls[propName].controls[i].controls[propName].setValue(
          value[i] == "none" ? "" : value[i]
        );
      }
    }
  }
