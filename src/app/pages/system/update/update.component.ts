...
export class UpdateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.product_type.includes("ENTERPRISE")) {
      setTimeout(() => {
        // To get around too many concurrent calls???
  ...
  applyFailoverUpgrade() {
    this.dialogService
      .confirm(T("Finish Upgrade?"), T(""), true, T("Continue"))
      .subscribe((res) => {
        if (res) {
  ...
  onTrainChanged(event) {
    ...
    this.dialogService
      .confirm(T("Switch Train"), warning + T("Switch update trains?"))
      .subscribe((train_res) => {
        if (train_res) {
          this.train = event;
        ...
        if (!this.ds.componentInstance.data[0].reboot) {
          ...
          this.dialogRef.componentInstance.success.subscribe((succ) => {
            ...
            this.dialogService.Info(
              T("Updates successfully downloaded"),
              "",
              "450px",
              "info",
              true
    ...
    if (!this.is_ha) {
      ...
      this.ws.call("update.set_train", [this.train]).subscribe(() => {
        ...
        this.dialogRef.componentInstance.success.subscribe((res) => {
          ...
          this.dialogService
            .confirm(
              helptext.ha_update.complete_title,
              helptext.ha_update.complete_msg,
              true,
              helptext.ha_update.complete_action,
              false,
              "",
              "",
              "",
            ...
            .subscribe(
              ...
              (err) => {
                ...
                entityDialog.parent.dialogService
                  .confirm(
                    helptext.save_config_err.title,
                    helptext.save_config_err.message,
                    false,
                    helptext.save_config_err.button_text
                  )
                  .subscribe((res) => {
                    if (res) {
                      entityDialog.parent.continueUpdate();
