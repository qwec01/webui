...
export class CloudCredentialsFormComponent {
    ...
    { name: "spacer", label: false, width: "2%" },
    {
      ...
      config: [
        ...
        },
        {
          ...
          placeholder:
            helptext.service_account_credentials_google_cloud_storage
              .placeholder,
          tooltip:
            helptext.service_account_credentials_google_cloud_storage.tooltip,
          required: true,
          isHidden: true,
          validation:
            helptext.service_account_credentials_google_cloud_storage
      ...
      function: () => {
        ...
        for (const item in value) {
          if (item != "name" && item != "provider") {
            ...
            if (
              item !== "preview-GOOGLE_CLOUD_STORAGE" &&
              item !== "advanced-S3" &&
              item !== "drives-ONEDRIVE"
            ) {
        ...
        if (
          value.attributes.private_key &&
          value.attributes.private_key === "NEW"
        ) {
  ...
  constructor(
    ...
    const authenticationFieldset = _.find(this.fieldSets, {
      class: "authentication",
    });
    const privateKeySFTPField = _.find(authenticationFieldset.config, {
      name: "private_key-SFTP",
    });
    this.ws
      .call("keychaincredential.query", [[["type", "=", "SSH_KEY_PAIR"]]])
      .subscribe((res) => {
        for (let i = 0; i < res.length; i++) {
          privateKeySFTPField.options.push({
            label: res[i].name,
            value: res[i].id,
          });
        }
  ...
  afterInit(entityForm: any) {
    ...
    entityForm.formGroup.controls[
      "service_account_credentials-GOOGLE_CLOUD_STORAGE"
    ].valueChanges.subscribe((value) => {
      entityForm.formGroup.controls["preview-GOOGLE_CLOUD_STORAGE"].setValue(
        value
      );
    });
    ...
    entityForm.formGroup.controls["private_key-SFTP"].valueChanges.subscribe(
      (res) => {
        if (res !== undefined) {
          const required = res === "" ? true : false;
    ...
    entityForm.formGroup.controls["drives-ONEDRIVE"].valueChanges.subscribe(
      (res) => {
        if (res) {
    ...
    const authCtrl =
      entityForm.formGroup.controls["auth_version-OPENSTACK_SWIFT"];
    const tenantCtrl = entityForm.formGroup.controls["tenant-OPENSTACK_SWIFT"];
    const tenantIdCtrl =
      entityForm.formGroup.controls["tenant_id-OPENSTACK_SWIFT"];
    entityForm.formGroup.controls[
      "auth_version-OPENSTACK_SWIFT"
      ...
      if (res === "1") {
        ...
        if (
          (tenantCtrl.value === undefined || tenantCtrl.value === "") &&
    ...
    entityForm.formGroup.controls[
      "tenant-OPENSTACK_SWIFT"
    ].valueChanges.subscribe((res) => {
      if (authCtrl.value !== "1") {
        this.setFieldRequired(
          "tenant_id-OPENSTACK_SWIFT",
          res === "" || res === undefined,
          entityForm
        );
      }
  ...
  async makeNewKeyPair(value, submitting?: boolean) {
    await this.replicationService.genSSHKeypair().then(
      async (res) => {
        ...
        await this.ws
          .call("keychaincredential.create", [payload])
          .toPromise()
          .then(
            (sshKey) => {
              this.keyID = sshKey.id;
              const privateKeySFTPField = _.find(this.fieldConfig, {
                name: "private_key-SFTP",
              });
              privateKeySFTPField.options.push({
                label: payload.name,
              ...
              this.entityForm.formGroup.controls["private_key-SFTP"].setValue(
                this.keyID
              );
              if (submitting) {
      ...
      if (item != "name" && item != "provider") {
        if (
          ...
          attributes[attr_name] =
            attr_name === "auth_version"
              ? parseInt(value[item], 10)
              : value[item];
        }
        delete value[item];
      }
  ...
  dataAttributeHandler(entityForm: any) {
    ...
    if (
      provider == "S3" &&
      (entityForm.wsResponseIdx["endpoint"] ||
        entityForm.wsResponseIdx["skip_region"] ||
        entityForm.wsResponseIdx["signatures_v2"])
    ) {
    ...
    for (let i in entityForm.wsResponseIdx) {
      ...
      if (entityForm.formGroup.controls[field_name]) {
        entityForm.formGroup.controls[field_name].setValue(
          field_name == "auth_version-OPENSTACK_SWIFT"
  ...
  getOnedriveList(data) {
    ...
    this.ws
      .call("cloudsync.onedrive_list_drives", [
        {
      ...
      .subscribe(
        (drives) => {
          for (let i = 0; i < drives.length; i++) {
            drivesConfig.options.push({
              label: drives[i].drive_type + " - " + drives[i].drive_id,
              value: drives[i],
            });
          }
        },
