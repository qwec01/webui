...
export class ServiceNFSComponent {
  ...
  private ipChoices$ = this.ws
    .call("nfs.bindip_choices", [])
    .pipe(
  ...
  addSPN() {
    ...
    if (!this.hasNfsStatus && this.adHealth === "HEALTHY") {
      this.dialog
        .confirm(
          helptext.add_principal_prompt.title,
          helptext.add_principal_prompt.message,
          true,
          helptext.add_principal_prompt.affirmative,
          false,
          "",
          "",
          "",
          "",
          false,
          helptext.add_principal_prompt.negative
        )
        .subscribe((res) => {
          if (res) {
                ...
                self.ws
                  ...
                  .subscribe(
                    ...
                    (err) => {
                      self.loader.close();
                      self.dialogRef.close(true);
                      that.dialog.errorReport(
                        helptext.add_principal_form.error_title,
                        err.reason,
                        err.trace.formatted
                      );
                    }
