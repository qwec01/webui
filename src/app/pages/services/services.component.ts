...
export class Services implements OnInit {
      ...
      if (service.title == "iscsitarget") {
        this.iscsiService.getGlobalSessions().subscribe((res) => {
          const msg =
            res.length == 0
              ? ""
              : T('<font color="red"> There are ') +
                res.length +
                T(
                  " active iSCSI connections.</font><br>Stop the " +
                    service.name +
        ...
        this.dialog
          .confirm(T("Alert"), T("Stop ") + service.name + "?", true, T("Stop"))
          .subscribe((res) => {
            if (res) {
              this.updateService(rpc, service);
            }
  ...
  updateService(rpc, service) {
    ...
    this.busy = this.ws.call(rpc, [service.title]).subscribe(
      (res) => {
        if (res) {
