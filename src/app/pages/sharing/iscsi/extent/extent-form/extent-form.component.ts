...
export class ExtentFormComponent {
  ...
  afterInit(entityForm: any) {
    ...
    this.avail_threshold_field = _.find(this.fieldConfig, {
      name: "avail_threshold",
    });
    ...
    this.extent_disk_control.valueChanges.subscribe((value) => {
      ...
      if (_.startsWith(value, "zvol")) {
        ...
        if (
          this.pk &&
          value != undefined &&
          _.find(extent_disk_field.options, { value: value }) === undefined
        ) {
  ...
  blurFilesize(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["filesize"].setValue(
