...
import {
  ...
  EmptyConfig,
  EmptyType,
} from "app/pages/common/entity/entity-empty/entity-empty.component";
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    core
      .register({ observerClass: this, eventName: "SidenavStatus" })
  ...
  init() {
    ...
    this.core
      .register({ observerClass: this, eventName: "NicInfo" })
      .subscribe((evt: CoreEvent) => {
          ...
          if (item.type == "LINK_AGGREGATION") {
            ...
            item.lag_ports.forEach((nic) => {
              ...
              clone[index].state.aliases = clone[index].state.aliases.concat(
                clone[nicKeys[nic]].state.aliases
              );

              // Consolidate vlans
              clone[index].state.vlans.forEach((item) => {
                item.interface = nic;
              });
              clone[index].state.vlans = clone[index].state.vlans.concat(
                clone[nicKeys[nic]].state.vlans
              );
        ...
        for (let i = clone.length - 1; i >= 0; i--) {
          if (removeNics[clone[i].name]) {
            ...
            clone[i].state.aliases = clone[i].state.aliases.filter(
              (address) => address.type == "INET" || address.type == "INET6"
            );
          }
        }
  ...
  startListeners() {
    this.core
      .register({ observerClass: this, eventName: "UserAttributes" })
      .subscribe((evt: CoreEvent) => {
  ...
  setVolumeData(evt: CoreEvent) {
    ...
    for (let i in evt.data) {
      if (typeof evt.data[i] == undefined || !evt.data[i]) {
        continue;
      }
  ...
  dataFromConfig(item: DashConfigItem) {
    ...
    switch (item.name.toLowerCase()) {
      ...
      case "pool":
        data = spl
          ? this.pools.filter((pool) => pool[key] == value)
      ...
      case "interface":
        data = spl
          ? this.nics.filter((nic) => nic[key] == value)
          : console.warn("DashConfigItem has no identifier!");
        if (data) {
          data = data[0].state;
        }
