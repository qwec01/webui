...
export class VolumesListTableConfig implements InputTableConf {
  ...
  constructor(
    ...
    if (
      typeof this._classId !== "undefined" &&
      this._classId !== "" &&
      volumeData &&
      volumeData["children"]
    ) {
  ...
  getEncryptedActions(rowData: any) {
    ...
    if (rowData.encrypt === 2) {
                  ...
                  fieldConfig: [
                    {
                      ...
                      paraText:
                        helptext.pool_lock_warning_paratext_a +
                        row1.name +
      ...
      rowData.encrypt === 1 &&
      rowData.is_decrypted &&
      self.parentVolumesListComponent.systemdatasetPool != rowData.name
    ) {
      actions.push({
    ...
    if (
      this.parentVolumesListComponent.has_encrypted_root[rowData.name] &&
  ...
  unlockAction(row1) {
    const self = this;
    this.storageService
      .poolUnlockServiceChoices(row1.id)
      .pipe(
        map((serviceChoices) => {
          return {
            ...
            afterInit: function (entityDialog) {
              self.message_subscription = self.messageService.messageSourceHasNewMessage$.subscribe(
                (message) => {
            ...
            customSubmit: function (entityDialog) {
              ...
              const params = [
                row1.id,
                {
                  ...
                  services_restart: value.services_restart,
                },
              ];
            ...
            this.dialogConf = {
              ...
              customSubmit: function (entityDialog) {
                ...
                const dialogRef = self.mdDialog.open(EntityJobComponent, {
                  data: {
                    title: helptext.pool_options_dialog.save_pool_options,
                  },
                  disableClose: true,
                });
                dialogRef.componentInstance.setDescription(
                  helptext.pool_options_dialog.saving_pool_options
                );
                ...
                this.ws.call("pool.processes", [row1.id]).subscribe(
                  (res) => {
                    ...
                    if (res.length > 0) {
                      ...
                      if (running_processes.length > 0) {
                        self.translate
                          .get(helptext.exportMessages.running)
                          .subscribe((runningMsg) => {
                      ...
                      if (running_unknown_processes.length > 0) {
                        self.translate
                          .get(helptext.exportMessages.unknown)
                          .subscribe((unknownMsg) => {
                            self.translate
                              .get(helptext.exportMessages.terminated)
                              .subscribe((terminatedMsg) => {
                                p1 += `<br><br>` + unknownMsg;
                                running_unknown_processes.forEach((process) => {
                                  if (process.pid) {
        ...
        actions.push({
          ...
          onClick: (row1) => {
            ...
            function doExpand(entityDialog?) {
              ...
              if (entityDialog) {
                payload.push({
                  geli: { passphrase: entityDialog.formValue["passphrase"] },
                });
              }
              parent.ws.job("pool.expand", payload).subscribe(
                (res) => {
                  ...
                  if (res.state === "SUCCESS") {
                    ...
                    self.translate
                      .get(helptext.expand_pool_success_dialog.message)
                      .subscribe((msg) => {
        ...
        actions.push(
          {
            ...
            onClick: (row1) => {
              this.ws
                .call("filesystem.acl_is_trivial", ["/mnt/" + rowData.id])
          ...
          onClick: (row1) => {
            self.translate.get("Delete Dataset").subscribe((msg1) => {
              self.translate.get("The").subscribe((theTr) => {
                self.translate
                  .get(
                    "dataset and all snapshots stored with it <b>will be permanently deleted</b>."
                  )
                  .subscribe((msg2) => {
                    this.dialogService
                      .doubleConfirm(
                  ...
                  parent.ws
                    ...
                    .subscribe((vmware_res) => {
                      ...
                      _.find(parent.dialogConf.fieldConfig, {
                        name: "vmware_sync",
                      })["isHidden"] = !parent.vmware_res_status;
                    });
                },
              },
              {
  ...
  getEncryptedDatasetActions(rowData) {
    ...
    if (rowData.encrypted) {
      if (rowData.locked) {
        if (
          rowData.is_encrypted_root &&
          (!rowData.parent || (rowData.parent && !rowData.parent.locked))
        ) {
        ...
        encryption_actions.push({
          ...
          onClick: (row) => {
            ...
            for (let i = 0; i < this.datasetData.length; i++) {
              ...
              if (
                ds["id"].startsWith(row.id) &&
                ds.id !== row.id &&
                ds.encryption_root &&
                ds.id === ds.encryption_root &&
                ds.key_format &&
                ds.key_format.value &&
                ds.key_format.value === "HEX"
              ) {
                key_child = true;
                break;
              }
            ...
            this.dialogConf = {
                ...
                },
                {
                  ...
                  value:
                    row.encryption_algorithm && row.encryption_algorithm.value
                      ? row.encryption_algorithm.value
                      : "",
                },
                {
                  type: "checkbox",
              ...
              afterInit: function (entityDialog) {
                ...
                const encryption_type_subscription = encryption_type_fg.valueChanges.subscribe(
                  (enc_type) => {
              ...
              customSubmit: function (entityDialog) {
                ...
                if (formValue.inherit_encryption) {
                  if (row.is_encrypted_root) {
                    ...
                    // just close the dialog if the inherit checkbox is checked but we are already inheriting
                    entityDialog.dialogRef.close();
                  }
                } else {
  ...
  getMoreDatasetInfo(dataObj, parent) {
    ...
    this.translate.get(T("Inherits")).subscribe((inherits) => {
      for (const k in dataset_data2) {
        ...
        dataObj.available_parsed = this.storageService.convertBytestoHumanReadable(
          dataObj.available.parsed || 0
        );
        dataObj.used_parsed = this.storageService.convertBytestoHumanReadable(
          dataObj.used.parsed || 0
        );
...
export class VolumesListComponent
  ...
  public entityEmptyConf: EmptyConfig = {
    ...
    large: true,
    title: T("No Pools"),
    message: `${T(`It seems you haven't configured pools yet.`)} ${T(
  ...
  constructor(
    ...
    this.actionsConfig = {
      actionType: VolumesListControlsComponent,
      actionConfig: this,
    };
    this.core.emit({
      name: "GlobalActions",
      data: this.actionsConfig,
      sender: this,
    });
  }
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.systemdatasetPool = await this.ws
      .call("systemdataset.config")
      .pipe(map((res) => res.pool))
      .toPromise();
      ...
      async ([pools, datasets]) => {
        if (pools.length > 0) {
          for (const pool of pools) {
            ...
            if (pChild) {
              pChild.children = pChild.children.filter(
                (child) =>
                  child.name.indexOf(`${pool.name}/.system`) === -1 &&
                  child.name.indexOf(`${pool.name}/.glusterfs`) === -1
              );
            }
            ...
            if (pool.children && pool.children[0]) {
              try {
                pool.children[0].is_encrypted_root =
                  pool.children[0].id === pool.children[0].encryption_root;
                ...
                for (let i = 0; i < datasets.length; i++) {
                  const ds = datasets[i];
                  if (
                    ds["id"].startsWith(pool.children[0].id) &&
                    ds.id !== pool.children[0].id &&
                    ds.encrypted
                  ) {
    ...
    this.addZvolComponent = new ZvolFormComponent(
      this.router,
      this.aroute,
      this.rest,
      this.ws,
