...
export class InterfacesFormComponent extends ViewControllerComponent implements OnDestroy {
  ...
  preInit(entityForm: any) {
    ...
    this.failover_fieldset = _.find(this.fieldSets, {
      class: "failover_settings",
    });
    this.vlan_pint = _.find(this.vlan_fieldset.config, {
      name: "vlan_parent_interface",
    });
  }
  ...
  afterInit(entityForm: any) {
    ...
    this.vlan_pint = _.find(this.fieldConfig, {
      name: "vlan_parent_interface",
    });
    ...
    if (window.localStorage.getItem("product_type").includes("ENTERPRISE")) {
      this.ws.call("failover.node").subscribe((node) => {
        ...
        _.find(this.ipListControl.templateListField, {
          name: "address",
        }).placeholder += this.ipPlaceholder;
        _.find(this.ipListControl.templateListField, {
          name: "failover_address",
        }).placeholder += this.failoverPlaceholder;
      });
    }
    ...
    this.offload_warning_sub = entityForm.formGroup.controls[
      "disable_offload_capabilities"
    ].valueChanges.subscribe((res) => {
      if (res && !this.offload_warned) {
        this.dialog
          .confirm(
            helptext.disable_offload_capabilities_warning_title,
            helptext.disable_offload_capabilities_warning_msg
          )
          .subscribe((confirm) => {
            if (confirm) {
    ...
    if (window.localStorage.getItem("product_type").includes("ENTERPRISE")) {
      this.ws.call("failover.licensed").subscribe((is_ha) => {
        ...
        if (is_ha) {
          this.aliases_subscription = this.entityForm.formGroup.controls[
            "aliases"
          ].valueChanges.subscribe((res) => {
            let v6_found = false;
  ...
  clean(data) {
    ...
    for (let i = 0; i < data.aliases.length; i++) {
      if (!data.aliases[i]["delete"] && !!data.aliases[i]["address"]) {
