  ...
  getChildren(node: any) {
    return new Promise((resolve, reject) => {
      if (this.config.explorerType === "zvol") {
        ...
        resolve(
          this.entityFormService.getFilesystemListdirChildren(
            node,
            this.config.explorerType,
            this.config.hideDirs
          )
        );
        ...
        resolve(
          this.entityFormService.getPoolDatasets(
            this.config.explorerParam ? this.config.explorerParam : []
          )
        );
      } else {
        resolve(this.entityFormService.getFilesystemListdirChildren(node));
