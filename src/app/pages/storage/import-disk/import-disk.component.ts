...
import { RestService, WebSocketService, JobService } from "../../../services/";
...
export class ImportDiskComponent implements OnDestroy, Formconfiguration {
  ...
  afterInit(entityForm: any) {
    ...
    this.ws.call("pool.import_disk_msdosfs_locales").subscribe(
      (res) => {
  ...
  makeList() {
    this.volume.options = [];
    this.ws.call("disk.get_unused", [true]).subscribe(
      (data) => {
  ...
  customSubmit(payload) {
    ...
    this.dialogRef = this.dialog.open(EntityJobComponent, {
      data: { title: T("Importing Disk") },
    });
    this.dialogRef.componentInstance.setDescription(T("Importing Disk..."));
    this.dialogRef.componentInstance.setCall("pool.import_disk", [
      payload.volume,
      payload.fs_type,
      fs_options,
      payload.dst_path,
