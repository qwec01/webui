 ...
 here when editing a user."
  ...
  afterInit(entityWizard: EntityWizardComponent) {
    ...
    (<FormGroup>entityWizard.formArray.get([0]))
      .get("path")
      .valueChanges.subscribe((pool: String) => {
  ...
  async customNext(stepper) {
    if (stepper._selectedIndex == 0) {
      if (!this.parent) {
        this.wizardConfig[0].fieldConfig.find(
          (c) => c.name === "path"
        ).warnings = `Please select a ZFS Volume`;
        return;
      } else {
