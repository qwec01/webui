...
export class VolumeImportWizardComponent {
  ...
  decryptDisks(stepper) {
    if (this.devices_fg.status === "INVALID") {
      this.dialogService.Info(
        T("Disk Selection Required"),
        T("Select one or more disks to decrypt.")
      );
      return;
    }
    if (!this.subs) {
      this.dialogService.Info(
        T("Encryption Key Required"),
        T("Select a key before decrypting the disks.")
      );
    }
    ...
    let dialogRef = this.dialog.open(EntityJobComponent, {
      data: { title: helptext.decrypt_disks_title },
      disableClose: true,
    });
  ...
  getImportableDisks() {
    let dialogRef = this.dialog.open(EntityJobComponent, {
      data: { title: helptext.find_pools_title },
      disableClose: true,
    });
    ...
    dialogRef.componentInstance.success.subscribe((res) => {
      if (res && res.result) {
        ...
        for (let i = 0; i < result.length; i++) {
          this.guid.options.push({
            label: result[i].name + " | " + result[i].guid,
            value: result[i].guid,
          });
        }
      }
  ...
  afterInit(entityWizard: EntityWizardComponent) {
    ...
    this.guid_subscription = (<FormGroup>(
      entityWizard.formArray.get([this.importIndex]).get("guid")
    ...
    if (!this.productType.includes("SCALE")) {
      this.message_subscription = this.messageService.messageSourceHasNewMessage$.subscribe(
        (message) => {
