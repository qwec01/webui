...
export class InitiatorFormComponent implements OnInit {
  ...
  public connectedInitiatorsTooltip =
    helptext_sharing_iscsi.initiator_form_tooltip_connected_initiators;
  public error;
  ...
  getConnectedInitiators() {
    this.ws.call("iscsi.global.sessions").subscribe(
      (res) => {
        this.connectedInitiators = _.unionBy(
          res,
          (item) => item["initiator"] && item["initiator_addr"]
        );
      },
      (err) => {
