...
export class EntityFormService {
  ...
  getPoolDatasets(param = []) {
    const nodes = [];
    return this.ws
      .call("pool.filesystem_choices", param)
      .toPromise()
      .then((res) => {
        for (let i = 0; i < res.length; i++) {
  ...
  phraseInputData(value: any, config: InputUnitConfig) {
    ...
    if (matchUnits && matchUnits[0] === unit) {
      ...
      if (config.allowUnits) {
        const singleUnit = _.endsWith(humanReableUnit, "S")
          ? humanReableUnit.substring(0, humanReableUnit.length - 1)
          : humanReableUnit;
