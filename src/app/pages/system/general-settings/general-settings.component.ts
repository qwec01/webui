...
export class GeneralSettingsComponent implements OnInit, OnDestroy {
  ...
  protected localizationComponent = new LocalizationFormComponent(
    this.language,
    this.ws,
    this.dialog,
    this.loader,
  ...
  protected guiComponent = new GuiFormComponent(
    this.router,
    this.language,
    this.ws,
    this.dialog,
    ...
    switch (name) {
      ...
      case "ntp":
        addComponent = id
          ...
          : new NTPServerFormComponent(this.modalService);
        break;
      default:
  ...
  doNTPDelete(server: any) {
    this.dialog
      .confirm(
        helptext.deleteServer.title,
        `${helptext.deleteServer.message} ${server.address}?`,
        false,
        helptext.deleteServer.message
      )
      .subscribe((res) => {
        if (res) {
          this.loader.open();
  ...
  saveConfigSubmit(entityDialog) {
    ...
    entityDialog.ws.call("system.info", []).subscribe(
      (res) => {
        let fileName = "";
        let mimetype;
        if (res) {
              ...
              entityDialog.parent.storage
                ...
                .subscribe(
                  ...
                  (err) => {
                    entityDialog.loader.close();
                    entityDialog.dialogRef.close();
                    entityDialog.parent.dialog.errorReport(
                      helptext.config_download.failed_title,
                      helptext.config_download.failed_message,
                      err.message
                    );
                  }
                );
