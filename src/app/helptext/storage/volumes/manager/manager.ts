...
export default {
 ...
 disks is not recommended."
  ...
  manager_disknumErrorConfirmMessage: T(
    "It is not recommended to create a pool with vdevs\
 containing different numbers of disks. Continue?"
  ),

  manager_disknumExtendConfirmMessage: T(
    "It is not recommended to extend a pool with one or\
 ...
 Override the following errors?"),
  ...
  spare_vdev_description: T(
    "Drive reserved for inserting into DATA pool vdevs when an active drive has failed."
  ),
  special_vdev_title: T("Metadata"),
  special_vdev_description: T(
    "Special Allocation class, used to create Fusion pools. Optional vdev type\
 which is used to speed up metadata and small block IO."
  ),
  dedup_vdev_title: T("Dedup"),
  dedup_vdev_description: T(
    "De-duplication tables are stored on this special vdev type. These vdevs must\
