...
export class WidgetNicComponent
  ...
  get vlanAddresses() {
    ...
    if (
      this.path[2].name == "empty" ||
      this.nicState.vlans.length == 0 ||
      !this.nicState.vlans[parseInt(this.path[2].index)]
    ) {
      return [];
    }
  ...
  ngOnDestroy() {
    this.core.emit({
      name: "StatsRemoveListener",
      data: { name: "NIC", obj: this },
    });
  ...
  vlanAliases(vlanIndex: string | number) {
    if (typeof vlanIndex == "string") {
      vlanIndex = parseInt(vlanIndex);
    }
  ...
  manageInterface(_interface) {
    const navigationExtras: NavigationExtras = {
      state: { editInterface: _interface.name },
    };
