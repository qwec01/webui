...
import {
  ViewChartComponent,
  ViewChartMetadata,
} from "app/core/components/viewchart/viewchart.component";
...
export class ViewChartGaugeComponent implements AfterViewInit, OnChanges {
  ...
  render() {
    ...
    text.attr("x", width / 2).attr("y", top + offsetY);

    subtext.attr("x", width / 2).attr("y", top + 24 + offsetY);
  ...
  update(value) {
    if (!document.hidden) {
      ...
      let txt = d3
        .select("#gauge-" + this.chartId + " text#text-value")
