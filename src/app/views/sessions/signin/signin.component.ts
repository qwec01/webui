...
export class SigninComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  usernameInput: ElementRef<HTMLElement>;
  ...
  get copyrightYear() {
    return window.localStorage && window.localStorage.buildtime
      ? this.localeService.getCopyrightYearFromBuildTime()
      : "";
  }
  ...
  loginToken() {
    ...
    if (middleware_token) {
      this.ws.login_token(middleware_token).subscribe((result) => {
    ...
    if (this.ws.token && this.ws.redirectUrl != undefined) {
      ...
      this.ws.login_token(this.ws.token).subscribe((result) => {
        this.loginCallback(result);
      });
    }
  }
  ...
  getHAStatus() {
    if (
      ...
      this.ws.call("failover.status").subscribe(
        (res) => {
          this.failover_status = res;
          this.ha_info_ready = true;
          if (res !== "SINGLE") {
            this.ws.call("failover.get_ips").subscribe(
              (ips) => {
            ...
            this.ws.call("failover.disabled_reasons").subscribe(
              (reason) => {
