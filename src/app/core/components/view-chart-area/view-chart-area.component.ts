...
// import { Report, ReportData } from 'app/pages/reportsdashboard/components/report/report.component';
...
export class ViewChartAreaComponent extends ViewComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  constructor(private core: CoreService, public themeService: ThemeService) {
    ...
    this.chartId = UUID.UUID().toString();
    this.controlUid = 'chart_' + this.chartId;
  ...
  renderGraph(option: any): void {
    ...
    const options = {
      ...
      series: () => {
        const s: any = {};
        this.data.legend.forEach((item: any) => {
  ...
  makeColumn(data: any/* ReportData */, legendKey: any): number[] {
    const result: any = [];
  ...
  protected makeTimeAxis(rd: any /* ReportData */): any[] {
