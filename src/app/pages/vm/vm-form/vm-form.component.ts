...
export class VmFormComponent {
      ...
      config: [
        ...
        },
        {
          ...
          options: [
            ...
            { label: helptext.time_utc_text, value: "UTC" },
          ],
        },
        {
          type: "select",
  ...
  resourceTransformIncomingRestData(wsResponse) {
    wsResponse["memory"] = this.storageService.convertBytestoHumanReadable(
      wsResponse["memory"] * 1048576,
      0
    );
    return wsResponse;
  }
  ...
  beforeSubmit(data) {
    if (data["memory"] !== undefined && data["memory"] !== null) {
      data["memory"] = Math.round(
        this.storageService.convertHumanStringToNum(data["memory"]) / 1048576
      );
    }
  }
}
