...
export class CoreService {
  ...
  public unregister(reg: Registration) {
    ...
    if (!reg.eventName) {
      ...
      for (var i = 0; i < this.dispatchTable.length; i++) {
        let registration = this.dispatchTable[i];
        if (
          registration.observerClass == reg.observerClass &&
          registration.eventName == reg.eventName
        ) {
          continue;
        } else {
          clone.push(registration);
  ...
  public emit(evt: CoreEvent) {
    ...
    if (
      this.debug &&
      this.debug_filter_eventName.length > 0 &&
      this.debug_filter_eventName == evt.name
    ) {
      console.log("*******************************************************");
