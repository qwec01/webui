...
export class PluginAddComponent implements OnInit {
  ...
  updateIpValidation() {
    ...
    if (
      this.formGroup.controls["dhcp"].value == false &&
      this.formGroup.controls["nat"].value == false
    ) {
      if (
        (this.formGroup.controls["ip4_addr"].value == "" ||
          this.formGroup.controls["ip4_addr"].value == undefined) &&
        (this.formGroup.controls["ip6_addr"].value == "" ||
          this.formGroup.controls["ip6_addr"].value == undefined)
      ) {
        if (ip4AddrField.required == false) {
          ip4AddrField.required = true;
          this.formGroup.controls["ip4_addr"].setValidators([
            Validators.required,
            regexValidator(this.networkService.ipv4_regex),
          ]);
          this.formGroup.controls["ip4_addr"].updateValueAndValidity();
        }
        if (ip6AddrField.required == false) {
          ip6AddrField.required = true;
          this.formGroup.controls["ip6_addr"].setValidators([
            Validators.required,
            regexValidator(this.networkService.ipv6_regex),
          ]);
  ...
  ngOnInit() {
    this.ip4_interfaceField = _.find(this.fieldConfig, {
      name: "ip4_interface",
    });
    this.ip4_netmaskField = _.find(this.fieldConfig, { name: "ip4_netmask" });
    this.ip6_interfaceField = _.find(this.fieldConfig, {
      name: "ip6_interface",
    });
      ...
      this.ws
        ...
        .subscribe(
          (defaults) => {
            ...
            if (
              !defaults.properties.hasOwnProperty("dhcp") &&
              !defaults.properties.hasOwnProperty("nat")
            ) {
  ...
  setRelation(config: FieldConfig) {
    ...
    if (activations) {
      ...
      this.fieldRelationService
        .getRelatedFormControls(config, this.formGroup)
        .forEach((control) => {
