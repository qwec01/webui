...
export class ChartEventsDialog implements OnInit {
  ...
  ngOnInit() {
    const chartQueryPromise = this.appService
      .getChartReleaseWithResources(this.catalogApp.name)
      .toPromise();
    const chartEventPromise = this.appService
      .getChartReleaseEvents(this.catalogApp.name)
      .toPromise();

    this.loader.open();
    Promise.all([chartQueryPromise, chartEventPromise]).then((res) => {
      this.loader.close();
      if (res[0]) {
  ...
  getUpdateVersionTooltip() {
    ...
    if (this.catalogApp.update_available) {
      ...
      const updateAvailableImages = Object.keys(this.containerImages).filter(
        (imageName) => this.containerImages[imageName].update_available
      );
