...
export class UserListComponent implements OnDestroy {
  ...
  refreshUserForm() {
    this.addComponent = new UserFormComponent(
      this.router,
      this.ws,
      this.storageService,
      this.loader,
  ...
  resourceTransformIncomingRestData(d) {
    ...
    this.ws.call("group.query").subscribe((res) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        rows[i].details = [];
        rows[i].details.push(
          { label: T("GID"), value: rows[i].group["bsdgrp_gid"] },
  ...
  toggleBuiltins() {
    let show;
    this.prefService.preferences.hide_builtin_users
