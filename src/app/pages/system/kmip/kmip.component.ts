...
export class KmipComponent {
  ...
  preInit() {
    const certificateFieldset = _.find(this.fieldSets, {
      class: "certificate",
    });
    const certificateField = _.find(certificateFieldset.config, {
      name: "certificate",
    });
    const certificateAuthorityField = _.find(certificateFieldset.config, {
      name: "certificate_authority",
    });

    this.systemGeneralService.getCA().subscribe((res) => {
      for (let i = 0; i < res.length; i++) {
        certificateAuthorityField.options.push({
          label: res[i].name,
          value: res[i].id,
        });
      }
    ...
    this.ws.call("kmip.sync_keys").subscribe(
      (res) => {
        this.dialogService.Info(
          helptext_system_kmip.syncInfoDialog.title,
          helptext_system_kmip.syncInfoDialog.info,
          "500px",
          "info",
          true
        );
  ...
  clearSyncKeys() {
    this.ws.call("kmip.clear_sync_pending_keys").subscribe(
      (res) => {
        this.dialogService.Info(
          helptext_system_kmip.clearSyncKeyInfoDialog.title,
          helptext_system_kmip.clearSyncKeyInfoDialog.info,
          "500px",
          "info",
          true
        );
