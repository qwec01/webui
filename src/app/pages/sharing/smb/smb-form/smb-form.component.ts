...
export class SMBFormComponent implements OnDestroy {
                      ...
                      if (doEnableService) {
                        ...
                        return this.ws
                          ...
                          .pipe(
                            ...
                            catchError((error) => {
                              entityForm.loader.close();
                              return this.dialog.errorReport(
                                error.error,
                                error.reason,
    ...
    this.ws.call("sharing.smb.presets").subscribe(
      (res) => {
        ...
        for (const item in res) {
          purposeField.options.push({
            label: res[item]["verbose_name"],
            value: item,
          });
        }
        if (entityForm.isNew) {
      ...
      setTimeout(() => {
        entityForm.formGroup.controls["aapl_name_mangling"].valueChanges.subscribe((value) => {
          if (value !== this.mangle && !this.mangleWarningSent) {
            ...
            this.dialog.confirm(
              helptext_sharing_smb.manglingDialog.title,
              helptext_sharing_smb.manglingDialog.message,
              true,
              helptext_sharing_smb.manglingDialog.action,
              false,
              null,
              null,
              null,
  ...
  showStripACLWarning() {
    this.dialog.confirm(
      helptext_sharing_smb.stripACLDialog.title,
      helptext_sharing_smb.stripACLDialog.message,
      true,
      helptext_sharing_smb.stripACLDialog.button,
      false,
      null,
      null,
      null,
