...
export class GroupQuotaFormComponent {
  ...
  afterInit(entityEdit: any) {
    ...
    this.entryField = _.find(
      this.fieldSets.find((set) => set.name === helptext.groups.group_title).config,
      { name: "searched_entries" }
    );

    this.ws.call("group.query").subscribe((res) => {
      res.map((entry) => {
        this.selectedEntriesField.options.push({
          label: entry.group,
          value: entry.gid,
        });
      });
  ...
  transformValue(parent, fieldname: string) {
    parent.entityForm.formGroup.controls[fieldname].setValue(
      parent.storageService.humanReadable || 0
    );
