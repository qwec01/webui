...
export class LocalizationFormComponent implements OnDestroy {
  ...
  getDateTimeFormats() {
    this.entityForm.formGroup.controls["date_format"].setValue(
      this.localeService.getPreferredDateFormat()
    );
    _.find(this.fieldConfig, { name: "date_format" })["isLoading"] = false;
    this.entityForm.formGroup.controls["time_format"].setValue(
      this.localeService.getPreferredTimeFormat()
    );
  ...
  makeLanguageList() {
    this.sysGeneralService.languageChoices().subscribe((res) => {
      ...
      let options = Object.keys(this.languageList || {}).map((key) => ({
