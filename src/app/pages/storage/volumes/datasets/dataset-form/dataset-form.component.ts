...
export class DatasetFormComponent implements Formconfiguration {
      ...
      config: [
        {
          ...
          validation: [
            (control: FormControl): ValidationErrors => {
              ...
              const size = this.convertHumanStringToNum(
                control.value,
                "refquota"
              );
              const errors =
              ...
              if (errors) {
                ...
                const size_err =
                  control.value && size != 0 && size < this.minrefquota
                    ? { invalid_size: true }
                    : null;
        ...
        },
        {
          ...
          validation: [
            (control: FormControl): ValidationErrors => {
              const config = this.fieldConfig.find(
                (c) => c.name === "refreservation"
              );

              const errors =
                control.value &&
                isNaN(
                  this.convertHumanStringToNum(control.value, "refreservation")
                )
                  ? { invalid_byte_string: true }
                  : null;
          ...
          validation: [
            (control: FormControl): ValidationErrors => {
              ...
              if (errors) {
                ...
                const size_err =
                  control.value && size != 0 && size < this.minquota
                    ? { invalid_size: true }
                    : null;
          ...
          validation: [
            (control: FormControl): ValidationErrors => {
              ...
              const errors =
                control.value &&
                isNaN(
                  this.convertHumanStringToNum(control.value, "reservation")
                )
                  ? { invalid_byte_string: true }
                  : null;
      ...
      config: [
        ...
        },
        {
          ...
          validation: [
            (control: FormControl): ValidationErrors => {
              const config = this.fieldConfig.find(
                (c) => c.name === "special_small_block_size"
              );
  ...
  convertHumanStringToNum(hstr, field) {
    const IECUnitLetters = this.storageService.IECUnits.map((unit) =>
  ...
  public sendAsBasicOrAdvanced(data: any): DatasetFormData {
    ...
    for (let i = 0; i < this.size_fields.length; i++) {
      ...
      if (this.OrigHuman[field] !== data[field]) {
        data[field] = Math.round(
          this.convertHumanStringToNum(data[field], field)
        );
  ...
  blurEventQuota(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["quota"].setValue(
        parent.humanReadable["quota"]
      );
    }
  }
  ...
  blurEventRefQuota(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["refquota"].setValue(
        parent.humanReadable["refquota"]
      );
    }
  }
  ...
  blurEventReservation(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["reservation"].setValue(
        parent.humanReadable["reservation"]
      );
    }
  }
  ...
  blurEventRefReservation(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["refreservation"].setValue(
        parent.humanReadable["refreservation"]
      );
    }
  }
  ...
  blurSpecialSmallBlocks(parent) {
    if (parent.entityForm) {
      parent.entityForm.formGroup.controls["special_small_block_size"].setValue(
        parent.humanReadable["special_small_block_size"]
      );
    }
  }
  ...
  afterInit(entityForm: EntityFormComponent) {
    ...
    if (!this.parent) {
      _.find(this.fieldConfig, { name: "quota_warning_inherit" }).placeholder =
        helptext.dataset_form_default;
      _.find(this.fieldConfig, { name: "quota_critical_inherit" }).placeholder =
        helptext.dataset_form_default;
      _.find(this.fieldConfig, {
        name: "refquota_warning_inherit",
      }).placeholder = helptext.dataset_form_default;
      _.find(this.fieldConfig, {
        name: "refquota_critical_inherit",
      }).placeholder = helptext.dataset_form_default;
    }
    if (!entityForm.isNew) {
      ...
      _.find(this.fieldConfig, { name: "name" }).tooltip =
    ...
    entityForm.formGroup
      .get("share_type")
      .valueChanges.pipe(filter((shareType) => !!shareType && entityForm.isNew))
      .subscribe((shareType) => {
        /* 
    ...
    this.recordsize_fg.valueChanges.subscribe((record_size) => {
      const record_size_number = parseInt(
        this.reverseRecordSizeMap[record_size],
        10
      );
      if (
        this.minimum_recommended_dataset_recordsize &&
        this.recommended_size_number
      ) {
        this.recordsize_warning =
          helptext.dataset_form_warning_1 +
          this.minimum_recommended_dataset_recordsize +
  ...
  preInit(entityForm: EntityFormComponent) {
    ...
    if (this.parent) {
      ...
      combineLatest(
        ...
        let inherit_encrypt_placeholder =
          helptext.dataset_form_encryption.inherit_checkbox_notencrypted;
        ...
        _.find(this.fieldConfig, {
          name: "inherit_encryption",
        }).placeholder = inherit_encrypt_placeholder;
        ...
        if (children.length > 0) {
          for (let i in children) {
            if (this.nameIsCaseInsensitive) {
              this.namesInUse.push(
                /[^/]*$/.exec(children[i].name)[0].toLowerCase()
              );
            } else {
        ...
        if (this.isNew) {
          if (this.legacy_encryption) {
            for (let i = 0; i < this.encryption_fields.length; i++) {
              ...
              _.find(this.fieldSets, {
                name: "encryption_divider",
              }).divider = false;
            }
            this.entityForm.setDisabled("encryption", true, true);
            this.entityForm.setDisabled("inherit_encryption", true, true);
            ...
            this.inherit_encryption_subscription = inherit_encryption_fg.valueChanges.subscribe(
              (inherit) => {
          ...
          const compression = _.find(this.fieldConfig, { name: "compression" });
          const deduplication = _.find(this.fieldConfig, {
            name: "deduplication",
          });
          ...
          const sync_inherit = [
            {
              label: `Inherit (${pk_dataset[0].sync.rawvalue})`,
              value: "INHERIT",
            },
          ];
          const compression_inherit = [
            {
              label: `Inherit (${pk_dataset[0].compression.rawvalue})`,
              value: "INHERIT",
            },
          ];
          const deduplication_inherit = [
            {
              label: `Inherit (${pk_dataset[0].deduplication.rawvalue})`,
              value: "INHERIT",
            },
          ];
          const exec_inherit = [
            {
              label: `Inherit (${pk_dataset[0].exec.rawvalue})`,
              value: "INHERIT",
            },
          ];
          const readonly_inherit = [
            {
              label: `Inherit (${pk_dataset[0].readonly.rawvalue})`,
              value: "INHERIT",
            },
          ];
          const atime_inherit = [
            {
              label: `Inherit (${pk_dataset[0].atime.rawvalue})`,
              value: "INHERIT",
            },
          ];
          this.storageService.convertHumanStringToNum(
            pk_dataset[0].recordsize.value
          );
          const recordsize_inherit = [
            {
              label: `Inherit (${this.storageService.humanReadable})`,
              value: "INHERIT",
            },
          ];
          if (
            pk_dataset[0].refquota_critical &&
            pk_dataset[0].refquota_critical.value
          ) {
            entityForm.formGroup.controls["refquota_critical"].setValue(
              pk_dataset[0].refquota_critical.value
            );
          }
          if (
            pk_dataset[0].refquota_warning &&
            pk_dataset[0].refquota_warning.value
          ) {
            entityForm.formGroup.controls["refquota_warning"].setValue(
              pk_dataset[0].refquota_warning.value
            );
          }
          if (
            ...
            entityForm.formGroup.controls["quota_critical"].setValue(
              pk_dataset[0].quota_critical.value
            );
          }
          if (
            pk_dataset[0].refquota_critical &&
            ...
            entityForm.formGroup.controls["quota_warning"].setValue(
              pk_dataset[0].quota_warning.value
            );
          }
          ...
          this.ws
            ...
            .subscribe((parent_dataset) => {
              this.parent_dataset = parent_dataset[0];
              const current_dataset = _.find(this.parent_dataset.children, {
                name: this.pk,
              });
              ...
              if (
                current_dataset.hasOwnProperty("recordsize") &&
                current_dataset["recordsize"].value
              ) {
                _.find(
                  _.find(this.fieldConfig, { name: "recordsize" }).options,
                  { value: current_dataset["recordsize"].value }
                )["hiddenFromDisplay"] = false;
              }
              const edit_sync = _.find(this.fieldConfig, { name: "sync" });
              const edit_compression = _.find(this.fieldConfig, {
                name: "compression",
              });
              const edit_deduplication = _.find(this.fieldConfig, {
                name: "deduplication",
              });
              const edit_exec = _.find(this.fieldConfig, { name: "exec" });
              const edit_readonly = _.find(this.fieldConfig, {
                name: "readonly",
              });
              const edit_atime = _.find(this.fieldConfig, { name: "atime" });
              const edit_recordsize = _.find(this.fieldConfig, {
                name: "recordsize",
              });
              let edit_sync_collection = [
                {
                  label: pk_dataset[0].sync.value,
                  value: pk_dataset[0].sync.value,
                },
              ];
              let edit_compression_collection = [
                {
                  label: pk_dataset[0].compression.value,
                  value: pk_dataset[0].compression.value,
                },
              ];
              let edit_deduplication_collection = [
                {
                  label: pk_dataset[0].deduplication.value,
                  value: pk_dataset[0].deduplication.value,
                },
              ];
              let edit_exec_collection = [
                {
                  label: pk_dataset[0].exec.value,
                  value: pk_dataset[0].exec.value,
                },
              ];
              let edit_readonly_collection = [
                {
                  label: pk_dataset[0].readonly.value,
                  value: pk_dataset[0].readonly.value,
                },
              ];
              let edit_atime_collection = [
                {
                  label: pk_dataset[0].readonly.value,
                  value: pk_dataset[0].readonly.value,
                },
              ];
              let edit_recordsize_collection = [
                {
                  label: this.parent_dataset.recordsize.value,
                  value: this.parent_dataset.recordsize.value,
                },
              ];

              edit_sync_collection = [
                {
                  label: `Inherit (${this.parent_dataset.sync.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_sync.options = edit_sync_collection.concat(
                edit_sync.options
              );

              edit_compression_collection = [
                {
                  label: `Inherit (${this.parent_dataset.compression.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_compression.options = edit_compression_collection.concat(
                edit_compression.options
              );

              edit_deduplication_collection = [
                {
                  label: `Inherit (${this.parent_dataset.deduplication.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_deduplication.options = edit_deduplication_collection.concat(
                edit_deduplication.options
              );

              edit_exec_collection = [
                {
                  label: `Inherit (${this.parent_dataset.exec.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_exec.options = edit_exec_collection.concat(
                edit_exec.options
              );

              edit_readonly_collection = [
                {
                  label: `Inherit (${this.parent_dataset.readonly.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_readonly.options = edit_readonly_collection.concat(
                edit_readonly.options
              );

              edit_atime_collection = [
                {
                  label: `Inherit (${this.parent_dataset.atime.rawvalue})`,
                  value: "INHERIT",
                },
              ];
              edit_atime.options = edit_atime_collection.concat(
                edit_atime.options
              );

              const lastChar = this.parent_dataset.recordsize.value[
                this.parent_dataset.recordsize.value.length - 1
              ];
              const formattedLabel =
                lastChar === "K" || lastChar === "M"
                  ? `${this.parent_dataset.recordsize.value.slice(
                      0,
                      -1
                    )} ${lastChar}iB`
                  : this.parent_dataset.recordsize.value;
              edit_recordsize_collection = [
                { label: `Inherit (${formattedLabel})`, value: "INHERIT" },
              ];
              edit_recordsize.options = edit_recordsize_collection.concat(
                edit_recordsize.options
              );
              ...
              if (
                pk_dataset[0].compression.source === "INHERITED" ||
                pk_dataset[0].compression.source === "DEFAULT"
              ) {
              ...
              if (
                pk_dataset[0].deduplication.source === "DEFAULT" ||
              ...
              if (
                pk_dataset[0].exec.source === "DEFAULT" ||
                pk_dataset[0].exec.source === "INHERITED"
              ) {
                exec_value = "INHERIT";
              }
              let readonly_value = pk_dataset[0].readonly.value;
              if (
                pk_dataset[0].readonly.source === "DEFAULT" ||
                pk_dataset[0].readonly.source === "INHERITED"
              ) {
                readonly_value = "INHERIT";
              }
              let atime_value = pk_dataset[0].atime.value;
              if (
                pk_dataset[0].atime.source === "DEFAULT" ||
                pk_dataset[0].atime.source === "INHERITED"
              ) {
                atime_value = "INHERIT";
              }
              let recordsize_value = pk_dataset[0].recordsize.value;
              if (
                pk_dataset[0].recordsize.source === "DEFAULT" ||
                pk_dataset[0].recordsize.source === "INHERITED"
              ) {
                recordsize_value = "INHERIT";
              }

              entityForm.formGroup.controls["deduplication"].setValue(
                deduplication_value
              );
              entityForm.formGroup.controls["exec"].setValue(exec_value);
              entityForm.formGroup.controls["readonly"].setValue(
                readonly_value
              );
              entityForm.formGroup.controls["atime"].setValue(atime_value);
              entityForm.formGroup.controls["recordsize"].setValue(
                recordsize_value
              );
              this.parent_dataset = parent_dataset[0];
            });
  ...
  resourceTransformIncomingRestData(wsResponse): any {
    if (
      wsResponse.special_small_block_size &&
      wsResponse.special_small_block_size.rawvalue === "0"
    ) {
    ...
    const quota_warning = this.getFieldValueOrNone(wsResponse.quota_warning)
      ...
      : this.warning;
    const quota_warning_inherit = this.isInherited(
      wsResponse.quota_warning,
      quota_warning
    );
    const quota_critical = this.getFieldValueOrNone(wsResponse.quota_critical)
      ...
      : this.critical;
    const quota_critical_inherit = this.isInherited(
      wsResponse.quota_critical,
      quota_critical
    );
    const refquota_warning = this.getFieldValueOrNone(
      wsResponse.refquota_warning
    )
      ? this.getFieldValueOrNone(wsResponse.refquota_warning)
      : this.warning;
    const refquota_warning_inherit = this.isInherited(
      wsResponse.refquota_warning,
      refquota_warning
    );
    const refquota_critical = this.getFieldValueOrNone(
      wsResponse.refquota_critical
    )
      ? this.getFieldValueOrNone(wsResponse.refquota_critical)
      : this.critical;
    const refquota_critical_inherit = this.isInherited(
      wsResponse.refquota_critical,
      refquota_critical
    );
    const sizeValues = {};
    ...
    const returnValue: DatasetFormData = {
      ...
      comments:
        wsResponse.comments === undefined
          ? wsResponse.comments
          : wsResponse.comments.source === "LOCAL"
  ...
  customSubmit(body) {
    ...
    return (this.isNew === true
      ...
      (restPostResp) => {
        ...
        this.ws
          .call("filesystem.acl_is_trivial", [parentPath])
          .subscribe((res) => {
            if (res === false) {
              this.dialogService
                .confirm(
                  helptext.afterSubmitDialog.title,
                  helptext.afterSubmitDialog.message,
                  true,
                  helptext.afterSubmitDialog.actionBtn,
                  false,
                  "",
                  "",
                  "",
                  "",
                  false,
                  helptext.afterSubmitDialog.cancelBtn
                )
                .subscribe((res) => {
                  if (res) {
                    this.ws
                      .call("filesystem.getacl", [parentPath])
                      .subscribe(({ acltype }) => {
                        if (acltype === "POSIX1E") {
