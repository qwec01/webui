...
export class CloudsyncFormComponent {
  ...
  public fieldSets: FieldSets = new FieldSets([
    {
      ...
      config: [
        ...
        },
        {
          ...
          tooltip:
            helptext.transfer_mode_warning_sync +
            " " +
            helptext.transfer_mode_warning_copy +
            " " +
  ...
  getBucketFolders(credential, bucket, node) {
    ...
    return this.ws
      .call("cloudsync.list_directory", [data])
      .toPromise()
      .then(
        (res) => {
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            if (res[i].IsDir) {
              ...
              child["subTitle"] = res[i].Decrypted
                ? `${res[i].Decrypted} (${res[i].Name})`
                : res[i].Name;
  ...
  async afterInit(entityForm) {
    ...
    this.cloudcredentialService.getCloudsyncCredentials().then((res) => {
      res.forEach((item) => {
        this.credentials.options.push({
    ...
    this.formGroup.controls["credentials"].valueChanges.subscribe((res) => {
      ...
      if (res != null) {
        this.credentials_list.forEach((item) => {
          if (item.id == res) {
            const targetProvider = _.find(this.providers, {
              name: item.provider,
            });
            if (targetProvider && targetProvider["buckets"]) {
              ...
              this.getBuckets(item).subscribe(
                ...
                (err) => {
                  ...
                  this.dialog
                    .confirm(
                      err.extra ? err.extra.excerpt : T("Error: ") + err.error,
                      err.reason,
                      true,
                      T("Fix Credential")
                    )
            ...
            const task_schema = _.find(this.providers, { name: item.provider })
              ? _.find(this.providers, { name: item.provider })["task_schema"]
              : [];
    ...
    this.formGroup
      ...
      .subscribe(() => {
        this.dialog.Info(
          ...
          helptext.resetTransferModeDialog.content,
          "500px",
          "info",
          true
        );
    ...
    this.formGroup.get("transfer_mode").valueChanges.subscribe((mode) => {
      const paragraph = entityForm.fieldConfig.find(
    ...
    if (data.bwlimit) {
      ...
      for (let i = 0; i < data.bwlimit.length; i++) {
        ...
        if (data.bwlimit[i].bandwidth != null) {
          const bw = (<any>window).filesize(data.bwlimit[i].bandwidth, {
            output: "object",
          });
