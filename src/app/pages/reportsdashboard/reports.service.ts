...
export class ReportsService implements OnDestroy {
  ...
  constructor(private ws: WebSocketService, private core: CoreService) {
    //@ts-ignore
    this.reportsUtils = new Worker("./reports-utils.worker", {
      type: "module",
    });

    core
      .register({ observerClass: this, eventName: "ReportDataRequest" })
      .subscribe((evt: CoreEvent) => {
        ws.call("reporting.get_data", [[evt.data.params], evt.data.timeFrame]).subscribe(
          (raw_res) => {
            let res;
            ...
            if (evt.data.report.name == "cputemp") {
              ...
              this.reportsUtils.postMessage({
                name: "ProcessCommandsAsReportData",
                data: repl,
                sender: evt.sender.chartId,
              });
            } else {
