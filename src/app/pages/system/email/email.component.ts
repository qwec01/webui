...
export class EmailComponent implements OnDestroy {
      ...
      config: [
        ...
        },
        {
          ...
          customEventMethod: () => {
            ...
            window.open(
              "https://freenas.org/oauth/gmail?origin=" +
  ...
  toggleSmtpAuthControls() {
    this.entityEdit.setDisabled(
      "user",
      !this.sendMailMethod.value || !this.smtp.value,
      !this.sendMailMethod.value || !this.smtp.value
    );
    this.entityEdit.setDisabled(
      "pass",
  ...
  saveConfigSubmit(emailConfig): void {
    ...
    if (
      emailConfig.pass &&
      typeof emailConfig.pass === "string" &&
      emailConfig.pass.trim() === ""
    ) {
    ...
    this.ws.call(this.updateCall, [emailConfig]).subscribe(
      () => {
