...
interface TimeData {
  // This is in WidgetChartComponent as well. Widgets eventually need to be updated to use this instead
  start: number;
  end: number;
  step: number;
  legend?: string;
...
export class ChartDataUtilsService {
  ...
  constructor(protected core: CoreService) {
    ...
    core
      .register({ observerClass: this, eventName: "ReportsHandleSources" })
      .subscribe((evt: CoreEvent) => {
        thread.postMessage(evt);
      });

    core
      .register({ observerClass: this, eventName: "ReportsHandleStats" })
      .subscribe((evt: CoreEvent) => {
        thread.postMessage(evt);
      });
  }
