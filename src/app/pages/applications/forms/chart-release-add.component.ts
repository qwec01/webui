...
export class ChartReleaseAddComponent implements OnDestroy {
  ...
  private summaryItems = [
    {
      ...
      label: helptext.chartForm.release_name.placeholder,
    },
    {
      step: 0,
      fieldName: "repository",
      label: helptext.chartForm.image.repo.placeholder,
    },
    {
      step: 0,
      fieldName: "tag",
      label: helptext.chartForm.image.tag.placeholder,
    },
    {
      step: 1,
      fieldName: "containerCommand",
      label: helptext.chartForm.container.command.placeholder,
    },
  ];
  ...
  parseSchema(catalogApp) {
    try {
      const gpuConfiguration = catalogApp.schema.questions.find(
        (question) => question.variable == "gpuConfiguration"
      );
      ...
      return this.dialogService.errorReport(
        ...
        helptext.chartForm.parseError.message
      );
    }
  }
  ...
  customSubmit(data) {
    ...
    if (
      ...
      data.containerEnvironmentVariables.length > 0 &&
      data.containerEnvironmentVariables[0].name
    ) {
    ...
    let pfList = [];
    if (
      data.portForwardingList &&
      data.portForwardingList.length > 0 &&
      data.portForwardingList[0].containerPort
    ) {
    ...
    if (
      ...
      data.hostPathVolumes.length > 0 &&
      data.hostPathVolumes[0].hostPath
    ) {
    ...
    let ext_interfaces = [];
    if (
      data.externalInterfaces &&
      data.externalInterfaces.length > 0 &&
      data.externalInterfaces[0].hostInterface
    ) {
    ...
    this.dialogRef = this.mdDialog.open(EntityJobComponent, {
      data: { title: helptext.installing },
      disableClose: true,
    });
