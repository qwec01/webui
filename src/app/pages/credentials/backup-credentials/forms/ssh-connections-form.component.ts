...
export class SshConnectionsFormComponent {
  ...
  async preInit() {
    if (this.rowNum) {
      ...
      _.find(this.fieldSets[0].config, {
        name: "setup_method",
      }).isHidden = true;
    } else {
    ...
    this.keychainCredentialService
      ...
      .toPromise()
      .then((res) => {
        const sshConnections = res
          .filter((item) => item.id != this.rowNum)
          .map((sshConnection) => sshConnection.name);
        this.namesInUse.push(...sshConnections);
        this.namesInUseConnection.push(...sshConnections);
      });
    const privateKeyField = _.find(this.fieldSets[1].config, {
      name: "private_key",
    });
    this.keychainCredentialService
      .getSSHKeys()
  ...
  isManualAuthFormValid() {
    return (
      this.entityForm.formGroup.controls["host"].valid &&
