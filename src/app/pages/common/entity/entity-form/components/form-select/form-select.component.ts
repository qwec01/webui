...
export class FormSelectComponent implements Field, AfterViewInit, AfterViewChecked {
    ...
    this.control.valueChanges.subscribe((evt) => {
      ...
      if (evt) {
        if (this.config.multiple && Array.isArray(evt)) {
          ...
          const newStates = this.config.options.map(
  ...
  ngAfterViewChecked() {
    if (
      !this.formReady &&
      typeof this.config.options !== "undefined" &&
      this.config.options &&
      this.config.options.length > 0
    ) {
      let keys = Object.keys(this.group.controls);
      let newStates = this.config.options.map(
        (item) => this.selectedValues.indexOf(item.value) !== -1
      );
      this.selectStates = newStates;
      this.updateValues();
  ...
  shouldAlertOnOption(option) {
    return this.config.alert
      ? this.config.alert.forValues.findIndex((v) => v == option.value) >= 0
      : false;
  }
}
