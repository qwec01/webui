...
export class WidgetPoolComponent
  ...
  get unhealthyDisks() {
    if (this.poolState && this.poolState.topology) {
      ...
      return {
        totalErrors: unhealthy.length /*errors.toString()*/,
        disks: unhealthy,
      };
    } else {
  ...
  public title: string =
    this.path.length > 0 && this.poolState && this.currentSlide !== "0"
      ? this.poolState.name
      : "Pool";
  ...
  ngAfterViewInit() {
    ...
    this.core
      .register({ observerClass: this, eventName: "MultipathData" })
  ...
  getAvailableSpace() {
    ...
    if (isNaN(this.volumeData.used)) {
      ...
      let usedObj = (<any>window).filesize(this.volumeData.used, {
        output: "object",
        exponent: 3,
      });
    ...
    if (isNaN(this.volumeData.avail)) {
      ...
      let availableObj = (<any>window).filesize(this.volumeData.avail, {
        output: "object",
        exponent: 3,
      });
    ...
    if (
      this.diskSize.charAt(this.diskSize.length - 2) === "." ||
      this.diskSize.charAt(this.diskSize.length - 2) === ","
    ) {
  ...
  updateSlide(
    ...
    if (direction == "forward") {
      ...
      this.path[parseInt(this.currentSlide)] = {
        name: "empty",
        template: this.empty,
      };
    }
