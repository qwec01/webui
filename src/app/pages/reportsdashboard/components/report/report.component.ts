...
export class ReportComponent
  ...
  constructor(
    ...
    this.core
      .register({
        observerClass: this,
        eventName: "ReportData-" + this.chartId,
      })
    ...
    this.core
      .register({
        observerClass: this,
        eventName: "LegendEvent-" + this.chartId,
      })
      .subscribe((evt: CoreEvent) => {
    ...
    this.getGenConfig = this.sysGeneralService.getGeneralConfig.subscribe(
      (res) => (this.timezone = res.timezone)
    );
  }
  ...
  private setupData(changes) {
    const zoom = this.zoomLevels[this.timeZoomIndex];
    const rrdOptions = this.convertTimespan(zoom.timespan);
    let identifier = changes.report.currentValue.identifiers
      ? changes.report.currentValue.identifiers[0]
      : null;
