...
export class TargetFormComponent {
  ...
  async prerequisite(): Promise<boolean> {
    ...
    const portalGroupField = _.find(targetGroupFieldset.config, {
      name: "groups",
    }).templateListField[0];
    const initiatorGroupField = _.find(targetGroupFieldset.config, {
      name: "groups",
    }).templateListField[1];
    const authGroupField = _.find(targetGroupFieldset.config, {
      name: "groups",
    }).templateListField[3];
        ...
        .then(
          (protalRes) => {
            for (let i = 0; i < protalRes.length; i++) {
              ...
              portalGroupField.options.push({
                label: label,
                value: protalRes[i].id,
              });
            }
            resolve(true);
          },
          ...
          (initiatorsRes) => {
            ...
            for (let i = 0; i < initiatorsRes.length; i++) {
              const optionLabel =
                initiatorsRes[i].id +
                " (" +
                (initiatorsRes[i].initiators.length === 0
                  ? "ALL Initiators Allowed"
                  : initiatorsRes[i].initiators.toString()) +
                ")";
              initiatorGroupField.options.push({
                label: optionLabel,
                value: initiatorsRes[i].id,
              });
            }
            resolve(true);
          },
