...
export class CertificatesDashComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.systemGeneralService.getUnsignedCertificates().subscribe((res) => {
      res.forEach((item) => {
        this.unsignedCAs.push({ label: item.name, value: parseInt(item.id) });
      });
    });
  }
        ...
        tableConf: {
          ...
          edit: function (row) {
            this.parent.modalService.open(
              "slide-in-form",
              this.parent.certificateEditComponent,
              row.id
            );
          },
        },
      ...
      },
      {
        ...
        tableConf: {
          ...
          add: function () {
            this.parent.modalService.open(
              "slide-in-form",
              this.parent.certificateAuthorityAddComponent
            );
          },
          edit: function (row) {
            this.parent.modalService.open(
              "slide-in-form",
              this.parent.certificateAuthorityEditComponent,
              row.id
            );
          },
          delete: function (row, table) {
            if (row.signed_certificates > 0) {
              this.parent.dialogService.confirm(
                helptext_system_ca.delete_error.title,
                helptext_system_ca.delete_error.message,
                true,
                helptext_system_ca.delete_error.button,
                false,
                "",
                "",
                "",
  ...
  refreshForms() {
    ...
    this.certificateAuthorityEditComponent = new CertificateAuthorityEditComponent(
      this.ws,
      this.loader,
      this.modalService,
      this.storage,
    ...
    this.acmeAddComponent = new CertificateAcmeAddComponent(
      this.ws,
      this.loader,
      this.dialog,
    ...
    this.acmeDNSComponent = new AcmednsFormComponent(
      this.ws,
      this.loader,
      this.dialogService,
      this.modalService
