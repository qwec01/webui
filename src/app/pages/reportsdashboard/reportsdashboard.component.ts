...
export class ReportsDashboardComponent
  implements OnInit, OnDestroy, /*HandleChartConfigDataFunc,*/ AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.core
      .register({ observerClass: this, eventName: "ReportingGraphs" })
      .subscribe((evt: CoreEvent) => {
        if (evt.data) {
  ...
  ngAfterViewInit(): void {
    ...
    this.actionsConfig = {
      actionType: ReportsGlobalControlsComponent,
      actionConfig: this,
    };
    this.core.emit({
      name: "GlobalActions",
  ...
  activateTab(name: string) {
    ...
    let reportCategories =
      name == "Disk"
        ? this.diskReports
        : this.otherReports.filter((report) => {
    ...
    this.fieldSets = [
      {
        ...
        config: [
          ...
          },
          {
            ...
            options: this.diskMetrics
              ? this.diskMetrics
              : [{ label: "None available", value: "negative" }], // eg. [{label:'temperature',value:'temperature'},{label:'operations', value:'disk_ops'}],
            required: true,
            multiple: true,
