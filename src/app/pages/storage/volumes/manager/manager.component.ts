...
export class ManagerComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    const conf: DialogFormConfiguration = {
      ...
      afterInit: function (entityDialog) {
        ...
        const setParatext = function (vdevs) {
          ...
          const size = (<any>window).filesize(parent.first_data_vdev_disksize, {
            standard: "iec",
          });
  ...
  getPoolData() {
    this.ws.call(this.queryCall, [[["id", "=", this.pk]]]).subscribe(
      (res) => {
        if (res[0]) {
          ...
          this.ws
            .call(this.datasetQueryCall, [[["id", "=", res[0].name]]])
            .subscribe((datasets) => {
              if (datasets[0]) {
                this.extendedAvailable = datasets[0].available.parsed;
                this.size = (<any>window).filesize(this.extendedAvailable, {
                  standard: "iec",
                });
              }
            });
  ...
  ngAfterViewInit() {
    ...
    this.ws.call("disk.get_unused", []).subscribe(
      (res) => {
        this.loader.close();
        ...
        for (let i in res) {
          ...
          res[i]["capacity"] = (<any>window).filesize(res[i]["size"], {
            standard: "iec",
          });
          const details = [];
          if (res[i]["rotationrate"]) {
            details.push({
              label: T("Rotation Rate"),
              value: res[i]["rotationrate"],
            });
          }
  ...
  addVdev(group, initial_values = {}) {
    ...
    setTimeout(() => {
      // there appears to be a slight race condition with adding/removing
    ...
    this.vdevComponents.forEach((vdev, i) => {
      if (vdev.group === "data") {
        ...
        if (data_vdev_disknum > 0) {
          if (
            data_vdev_disknum !== this.first_data_vdev_disknum &&
            this.first_data_vdev_type !== "stripe"
          ) {
  ...
  getDuplicableDisks() {
    ...
    for (let i = 0; i < this.disks.length; i++) {
      ...
      if (
        disk.size === this.first_data_vdev_disksize &&
        disk.type === this.first_data_vdev_disktype
      ) {
  ...
  updateFilter(event) {
    ...
    if (re) {
      ...
      const temp = this.temp.filter(function (d) {
        return (
          self.nameFilter.test(d.devname.toLowerCase()) &&
