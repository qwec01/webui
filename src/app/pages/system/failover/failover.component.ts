...
export class FailoverComponent implements OnDestroy {
  ...
  public custActions: Array<any> = [
    {
      ...
      function: () => {
        ...
        const ds = this.dialog.confirm(
          ...
          false,
          helptext_system_failover.dialog_button_ok,
          true,
    ...
    },
    {
      ...
      function: () => {
        this.dialog
          .confirm(
            helptext_system_failover.dialog_sync_from_peer_title,
            helptext_system_failover.dialog_sync_from_peer_message,
            false,
            helptext_system_failover.dialog_button_ok
          )
          .subscribe((confirm) => {
            if (confirm) {
              ...
              this.ws.call("failover.sync_from_peer").subscribe(
                (res) => {
                  ...
                  this.dialog.Info(
                    helptext_system_failover.confirm_dialogs.sync_title,
                    helptext_system_failover.confirm_dialogs.sync_from_message,
                    "",
                    "info",
                    true
                  );
  ...
  afterInit(entityEdit: any) {
    ...
    this.failoverDisableSubscription = this.entityForm.formGroup.controls[
      "disabled"
    ...
    this.masterSubscription = this.master_fg.valueChanges.subscribe((res) => {
      if (!res && !this.warned) {
        this.dialog
          .confirm(
            helptext_system_failover.master_dialog_title,
            helptext_system_failover.master_dialog_warning,
            false,
            T("Continue"),
            false,
            "",
