...
export class BootEnvironmentListComponent implements OnDestroy {
  @ViewChild("scrubIntervalEvent", { static: true })
  scrubIntervalEvent: ElementRef;
  ...
  doActivate(id) {
    this.dialog
      .confirm(
        T("Activate"),
        T("Activate this Boot Environment?"),
        false,
        helptext_system_bootenv.list_dialog_activate_action
      )
      .subscribe((res) => {
        if (res) {
          this.loader.open();
  ...
  updateBootState(): void {
    this.ws.call("boot.get_state").subscribe((wres) => {
      ...
      this.size_consumed = this.storage.convertBytestoHumanReadable(
        wres.properties.allocated.parsed
      );
  ...
  toggleKeep(id, status) {
    if (!status) {
      this.dialog
        .confirm(
          T("Keep"),
          T("Keep this Boot Environment?"),
          false,
          helptext_system_bootenv.list_dialog_keep_action
        )
        .subscribe((res) => {
          if (res) {
            this.loader.open();
      ...
      this.dialog
        .confirm(
          T("Unkeep"),
          T("No longer keep this Boot Environment?"),
          false,
          helptext_system_bootenv.list_dialog_unkeep_action
        )
        .subscribe((res) => {
          if (res) {
            this.loader.open();
  ...
  goToStatus() {
    this._router.navigate(new Array("").concat(["system", "boot", "status"]));
  }
  ...
  scrub() {
    this.dialog
      .confirm(
        T("Scrub"),
        T("Start the scrub now?"),
        false,
        helptext_system_bootenv.list_dialog_scrub_action
      )
      .subscribe((res) => {
        if (res) {
          this.loader.open();
