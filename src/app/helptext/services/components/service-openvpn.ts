...
export default {
  ...
  client: {
    ...
    nobind: {
      ...
      tooltip: T(
        "Enable to prevent binding to local address and port. \
 Must be enabled if OpenVPN client and server are to run concurrently."
    ...
    tls_crypt_auth: {
      ...
      tooltip: T(
        "Provide static key for authentication/encryption of all control \
 channel packets when <code>tls_crypt_auth_enabled</code> is enabled."
  ...
  server: {
    ...
    tls_crypt_auth: {
      placeholder: T("TLS Crypt Auth"),
      tooltip: T(
        "When <code>tls_crypt_auth_enabled</code> is enabled and <code>tls_crypt_auth</code> is not provided, \
 a static key is automatically generated to be used with OpenVPN client."
    ...
    topology: {
      ...
      tooltip: T(
        "Configure virtual addressing topology when running in TUN mode. \
 (TAP mode always uses a SUBNET topology.)"
  ...
  certificate: {
    ...
    tooltip: T(
      'Choose a valid client certificate which exists \
...
target="_blank">here.</a>'
  ...
  root_ca: {
    ...
    tooltip: T(
      'Choose the root Certificate Authority that was used to sign the Client and Server certificates. \
 ...
 <a href="https://community.openvpn.net/openvpn/wiki/HOWTO#SettingupyourownCertificateAuthorityCAandgeneratingcertificatesandkeysforanOpenVPNserverandmultipleclients" target="_blank">here.</a>'
  ...
  device_type: {
    ...
    tooltip: T(
      'Choose a virtual network interface. More information can be found \
