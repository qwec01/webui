...
export class TopbarComponent
  ...
  ngOnInit() {
    ...
    this.ws.subscribe("core.get_jobs").subscribe((res) => {
      if (
        (res && res.fields.method === "update.update") ||
        res.fields.method === "failover.upgrade"
      ) {
    ...
    this.core
      .register({ observerClass: this, eventName: "ThemeListsChanged" })
    ...
    this.core
      .register({ observerClass: this, eventName: "UserPreferences" })
      .subscribe((evt: CoreEvent) => {
    ...
    this.ws.call("truenas.is_eula_accepted").subscribe((eula_accepted) => {
      if (
        ...
        this.ws.call("truenas.get_eula").subscribe((eula) => {
          this.dialogService
            .confirm(
              T("End User License Agreement - TrueNAS"),
              eula,
              true,
              T("I Agree"),
              false,
    ...
    if (this.router.url !== "/network") {
      this.dialogService
        .confirm(
          ...
          true,
          network_interfaces_helptext.checkin_button
        )
        .subscribe((res) => {
          if (res) {
  ...
  onShowTaskManager() {
    ...
    this.taskDialogRef.afterClosed().subscribe((res) => {
  ...
  updateTC() {
    ...
    let updateDialog;
    const conf: DialogFormConfiguration = {
      title: self.tcConnected
        ? helptext.updateDialog.title_update
        : helptext.updateDialog.title_connect,
      fieldConfig: [
        {
          type: "input",
          name: "api_key",
      ...
      saveButtonText: self.tcConnected
        ? helptext.updateDialog.save_btn
        : helptext.updateDialog.connect_btn,
      parent: this,
          ...
          (res) => {
            ...
            if (!self.tcConnected) {
              self.dialogService.Info(
                helptext.checkEmailInfoDialog.title,
                helptext.checkEmailInfoDialog.message
              );
            }
          },
          (err) => {
            ...
            new EntityUtils().handleWSError(
              entityDialog.parent,
              err,
              entityDialog.parent.dialogService
            );
          }
        );
      },
  ...
  openStatusDialog() {
    ...
    this.tcStatusDialogRef.afterClosed().subscribe((res) => {
