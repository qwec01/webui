...
export class AvailablePluginsComponent implements OnInit {
          ...
          this.ws
            ...
            .subscribe((community) => {
              this.ws
                .job(this.queryCall, [
                  {
                    plugin_repository: this.availableRepo[1]["git_repository"],
                  },
                ])
                .subscribe((official) => {
  ...
  getInstances() {
    this.ws.call("plugin.query").subscribe((res) => {
    ...
    if (!plugin.official) {
      this.parent.dialogService
        .confirm(
          T("Warning"),
          T(
            "This is an unofficial plugin not produced or supported by iXsystems. iXsystems does\
          ...
          if (res) {
            this.router.navigate(
              new Array("").concat([
                ...
                "add",
                plugin.plugin,
                { plugin_repository: this.selectedRepo },
              ])
            );
          }
        });
