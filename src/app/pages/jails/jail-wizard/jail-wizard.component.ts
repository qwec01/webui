...
export class JailWizardComponent {
  ...
  preInit() {
    this.releaseField = _.find(this.wizardConfig[0].fieldConfig, {
      name: "release",
    });
    ...
    this.ip4_interfaceField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "ip4_interface",
    });
    this.ip4_netmaskField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "ip4_netmask",
    });
    this.ip6_interfaceField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "ip6_interface",
    });
    this.ip6_prefixField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "ip6_prefix",
    });
      ...
      if (ip4_address_control.value == undefined || ip4_address_control.value == "") {
        ...
        if (ip4_interface_control.value != "") {
          const validInterface =
            _.find(this.ip4_interfaceField.options, {
              value: ip4_interface_control.value,
            }) !== undefined;
          full_address = validInterface
            ? ip4_interface_control.value + "|" + ip4_address_control.value
  ...
  async afterInit(entityWizard: EntityWizardComponent) {
    ...
    const httpsField = _.find(this.wizardConfig[0].fieldConfig, {
      name: "https",
    });
