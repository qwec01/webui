...
export class EntityTableRowDetailsComponent implements OnInit, OnChanges {
  ...
  buildColumns(): void {
    this.columns = this.parent.allColumns.filter(
      (col) => !this.parent.conf.columns.some((c) => c.prop === col.prop)
    );
  }

  getActions(): EntityTableAction[] {
    return this.parent.conf.getActions
      ? this.parent.conf.getActions(this.config)
      : this.parent.getActions(this.config);
  }
}
