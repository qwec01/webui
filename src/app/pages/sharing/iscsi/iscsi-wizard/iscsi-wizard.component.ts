...
export class IscsiWizardComponent {
  ...
  step0Init() {
    const disk_field = _.find(this.wizardConfig[0].fieldConfig, {
      name: "disk",
    });
    //get device options
    ...
    const taregt_field = _.find(this.wizardConfig[0].fieldConfig, {
      name: "target",
    });
    this.iscsiService.getTargets().subscribe((res) => {
  ...
  step1Init() {
    const authGroupField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "discovery_authgroup",
    });
    const listenIpField = _.find(this.wizardConfig[1].fieldConfig, {
      name: "listen",
    }).templateListField[0];

    this.iscsiService.listPortals().subscribe((portals) => {
      const field = _.find(this.wizardConfig[1].fieldConfig, {
        name: "portal",
      });
    ...
    this.iscsiService.getIpChoices().subscribe((ips) => {
      ...
      const listenListFields = _.find(this.wizardConfig[1].fieldConfig, {
        name: "listen",
      }).listFields;
  ...
  getSummary() {
    const summary = {
      ...
      Extent: {
        ...
        "New Device":
          this.summaryObj.dataset +
          "/" +
          this.summaryObj.name +
          "(" +
          this.summaryObj.volsize +
          " " +
          this.summaryObj.volsize_unit +
          ")",
      ...
      "New Portal": {
        ...
        "Discovery Auth Group":
          this.summaryObj.discovery_authgroup === "NEW"
            ? `${this.summaryObj.tag} (New Create)`
            : this.summaryObj.discovery_authgroup,
        Listen:
    ...
    if (this.summaryObj.type === "FILE") {
      ...
      this.summaryObj.disk === "Create New"
        ? delete summary["Extent"]["Device"]
        : delete summary["Extent"]["New Device"];
    }

    this.summaryObj.portal === "Create New"
  ...
  getDatasetValue(dataset) {
    const datasetField = _.find(this.wizardConfig[0].fieldConfig, {
      name: "dataset",
    });
      ...
      if (!toStop) {
        if (
          !(
            ...
            ((item === "initiator" || item === "portal" || item === "target") &&
              value["target"] !== "NEW")
          )
        ) {
  ...
  doCreate(value, item) {
    ...
    if (item === "extent") {
      ...
      payload = Object.assign(
        payload,
        _.find(this.defaultUseforSettings, { key: value["usefor"] }).values
      );
    }
    if (item === "initiator") {
      payload = {
  ...
  rollBack(items) {
    for (const item in items) {
      if (items[item] != null) {
        this.ws.call(this.deleteCalls[item], [items[item]]).subscribe((res) => {
  ...
  blurFilesize(parent) {
    if (parent.entityWizard) {
      parent.entityWizard.formArray.controls[0].controls["filesize"].setValue(
