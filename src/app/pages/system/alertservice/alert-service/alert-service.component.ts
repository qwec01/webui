...
export class AlertServiceComponent {
  ...
  dataAttributeHandler(entityForm: any) {
    ...
    for (const i in entityForm.wsResponseIdx) {
      ...
      if (entityForm.formGroup.controls[field_name]) {
        if (
          (i === "v3_authprotocol" || i === "v3_privprotocol") &&
          entityForm.wsResponseIdx[i] === null
        ) {
  ...
  generatePayload(data) {
    ...
    for (const i in data) {
      if (i === "name" || i === "type" || i === "enabled" || i === "level") {
        ...
        if (
          data[i] === "" &&
          (i === "SNMPTrap-v3_authprotocol" || i === "SNMPTrap-v3_privprotocol")
        ) {
          data[i] = null;
  ...
  getErrorField(field) {
    return _.find(this.fieldConfig, {
      name: this.entityForm.formGroup.controls["type"].value + "-" + field,
    });
  }
}
