...
export class VolumeStatusComponent implements OnInit {
  ...
  getData() {
    this.ws.call("pool.query", [[["id", "=", this.pk]]]).subscribe(
      (res) => {
        this.pool = res[0];
        if (res[0]) {
          // if pool is passphrase protected, abled passphrase field.
          if (res[0].encrypt === 2) {
            [this.replaceDiskFormFields, this.extendVdevFormFields].forEach(
              (formFields) => {
      ...
      for (const i in res) {
        ...
        availableDisksForExtend.push({
          label:
            res[i].devname +
            " (" +
            (<any>window).filesize(res[i].size, { standard: "iec" }) +
            ")",
          value: res[i].name,
        });
      }
      _.find(this.replaceDiskFormFields, {
        name: "disk",
      }).options = availableDisks;
      _.find(this.extendVdevFormFields, {
        name: "new_disk",
      }).options = availableDisksForExtend;
    });
  }
    ...
    const actions = [
      {
        ...
        onClick: (row) => {
          ...
          const diskName =
            pIndex > -1 ? row.name.substring(0, pIndex) : row.name;

          this.ws
            .call("disk.query", [[["devname", "=", diskName]]])
            .subscribe((res) => {
        ...
        onClick: (row) => {
          ...
          this.dialogService
            .confirm(
              ...
              helptext.offline_disk.message +
                name +
                "?" +
                (this.pool.encrypt == 0
                  ? ""
                  : helptext.offline_disk.encryptPoolWarning),
              false,
      ...
      },
      {
        ...
        isHidden:
          data.status == "ONLINE" || this.pool.encrypt !== 0 ? true : false,
      },
      {
        id: "replace",
        ...
        onClick: (row) => {
          ...
          const pk = this.pk;
          _.find(this.replaceDiskFormFields, { name: "label" }).value =
            row.guid;
          ...
          const conf: DialogFormConfiguration = {
            ...
            customSubmit: function (entityDialog: any) {
              ...
              const dialogRef = entityDialog.parent.matDialog.open(
                EntityJobComponent,
                {
              ...
              dialogRef.componentInstance.setDescription(
                helptext.replace_disk.description
              );
              dialogRef.componentInstance.setCall("pool.replace", [
                pk,
                entityDialog.formValue,
              ]);
              ...
              dialogRef.componentInstance.success.subscribe((res) => {
                ...
                entityDialog.parent.dialogService.Info(
                  ...
                  helptext.replace_disk.info_dialog_content + name + ".",
                  "",
                  "info",
                  true
                );
                ...
                dialogRef.componentInstance.failure.subscribe((res) => {
                  ...
                  entityDialog.parent.dialogService.errorReport(
                    helptext.replace_disk.err_title,
                    err,
                    res.exception
                  );
                });
          ...
          this.dialogService
            ...
            .subscribe((res) => {
              if (res) {
                this.loader.open();
                this.ws
                  .call("pool.remove", [this.pk, { label: row.guid }])
                  .subscribe(
                    (res) => {
                      this.getData();
                      this.loader.close();
          ...
          this.dialogService
            ...
            .subscribe((res) => {
              if (res) {
                this.loader.open();
                this.ws
                  .call("pool.detach", [this.pk, { label: row.guid }])
                  .subscribe(
                    (res) => {
                      this.getData();
                      this.getUnusedDisk();
    ...
    return [
      {
        ...
        onClick: (row) => {
          const pk = this.pk;
          _.find(this.extendVdevFormFields, { name: "target_vdev" }).value =
            row.guid;
          const conf: DialogFormConfiguration = {
            ...
            customSubmit: function (entityDialog: any) {
              ...
              dialogRef.componentInstance.setDescription(
                helptext.extend_disk.description
              );
              dialogRef.componentInstance.setCall("pool.attach", [
                pk,
                entityDialog.formValue,
              ]);
              dialogRef.componentInstance.submit();
              dialogRef.componentInstance.success.subscribe((res) => {
                ...
                entityDialog.parent.dialogService.Info(
                  ...
                  helptext.extend_disk.info_dialog_content + name + ".",
                  "",
                  "info",
                  true
                );
                ...
                dialogRef.componentInstance.failure.subscribe((res) => {
                  ...
                  entityDialog.parent.dialogService.errorReport(
                    helptext.extend_disk.err_title,
                    err,
                    res.exception
                  );
                });
    ...
    if (category && data.type) {
      if (data.type == "DISK") {
        item.actions = [
          {
            title: "Disk Actions",
            actions: this.getAction(data, category, vdev_type),
          },
        ];
      } else if (data.type === "MIRROR") {
        item.actions = [
          { title: "Mirror Actions", actions: this.extendAction(data) },
        ];
      }
    }
    return item;
  ...
  parseTopolgy(data: any, category: any, vdev_type?: any): TreeNode {
    ...
    if (data.children) {
      ...
      for (let i = 0; i < data.children.length; i++) {
        node.children.push(
          this.parseTopolgy(data.children[i], category, vdev_type)
        );
      }
    }
  ...
  dataHandler(pool: any) {
    ...
    for (const category in pool.topology) {
      ...
      for (let i = 0; i < pool.topology[category].length; i++) {
        if (category != "data") {
          topoNode.children.push(
            this.parseTopolgy(pool.topology[category][i], category)
          );
        } else {
          node.children.push(
            this.parseTopolgy(pool.topology[category][i], category)
  ...
  onClickEdit(pk) {
    let diskForm = new DiskFormComponent(
      this.router,
      this.rest,
      this.ws,
      this.aroute
