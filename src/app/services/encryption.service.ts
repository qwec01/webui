...
export class EncryptionService {
  ...
  setPassphrase(
    ...
    this.loader.open();
    this.ws
      .call("pool.passphrase", [
        parseInt(row),
        { passphrase: encryptKeyPassphrase, admin_password: adminPassphrase },
      ])
      .subscribe(
        () => {
          this.loader.close();
          this.dialogService.Info(
            T("Set Passphrase"),
            T(`Passphrase ${success_message} <i>${poolName}</i>`),
            "300px",
            "info",
            true
          );
        ...
        (err) => {
          ...
          this.dialogService.errorReport(
            T(`Error creating passphrase for pool ${poolName}`),
            err.reason,
            err.trace.formatted
          );
        }
  ...
  openEncryptDialog(row, route_success, poolName, addRecoveryKey?) {
    let dialogRef = this.mdDialog.open(DownloadKeyModalDialog, {
      disableClose: true,
    });
  ...
  deleteRecoveryKey(row, adminPassphrase, poolName, route_success) {
    this.dialogService
      .confirm(
        ...
        helptext.delete_recovery_key_message,
        true,
        T("Delete Key")
      )
      .subscribe((res) => {
          ...
          this.ws
            ...
            .subscribe(
              () => {
                ...
                this.dialogService.Info(
                  helptext.delete_recovery_key_title,
                  T(`Recovery key deleted from pool <i>${poolName}</i>`),
                  "300px",
                  "info",
                  true
                );
              ...
              (err) => {
                this.loader.close();
                this.dialogService.errorReport(
                  T(`Error deleting recovery key for pool ${poolName}`),
                  err.error.message,
                  err.error.traceback
                );
              }
