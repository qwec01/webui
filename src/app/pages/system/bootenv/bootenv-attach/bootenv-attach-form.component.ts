...
export class BootEnvAttachFormComponent {
  ...
  customSubmit(entityForm) {
    ...
    this.dialogRef.componentInstance.success.subscribe(() => {
      ...
      this.dialogService
        .Info(
          helptext_system_bootenv.attach_dialog.title,
          `<i>${entityForm.dev}</i> ${helptext_system_bootenv.attach_dialog.message}`,
          "300px",
          "info",
          true
        )
        .subscribe(() => {
          this.router.navigate(new Array("").concat("system", "boot"));
        });
    });
