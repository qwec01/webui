...
export class ThemeService {
  ...
  setCssVars(theme: Theme): void {
    ...
    palette.forEach((color) => {
      ...
      if (this.freenasThemes[0].accentColors.indexOf(color) !== -1) {
        const txtColor = this.utils.textContrast(swatch, theme.colors['bg2']);
    ...
    const primaryColor = this.utils.colorFromMeta(theme.colors['primary']); // eg. blue
    const accentColor = this.utils.colorFromMeta(theme.colors['accent']); // eg. yellow
    const primaryTextColor = this.utils.textContrast((theme as any)[primaryColor], theme.colors['bg2']);
    const accentTextColor = this.utils.textContrast((theme as any)[accentColor], theme.colors['bg2']);
    ...
    // Set line colors
    const isDark: boolean = this.darkTest(theme.colors.bg2);
    ...
    const contrastSrc = theme.colors['bg2'];
    ...
    if (!theme['topbar-txt'] && theme.topbar) {
      topbarTextColor = this.utils.textContrast(theme.topbar, theme.colors['bg2']);
      document.documentElement.style.setProperty('--topbar-txt', topbarTextColor);
    } else if (!theme['topbar-txt'] && !theme.topbar) {
      topbarTextColor = this.utils.textContrast((theme as any)[primaryColor], theme.colors['bg2']);
      document.documentElement.style.setProperty('--topbar-txt', topbarTextColor);
    }
  ...
  isDarkTheme(name: string = this.activeTheme): boolean {
    ...
    return this.darkTest(theme.colors.bg2);
  }
}
