...
export class ZvolFormComponent implements Formconfiguration {
    ...
    combineLatest(
      ...
      this.translate.get("Inherit").subscribe((inheritTr) => {
        if (pk_dataset && pk_dataset[0].type === "FILESYSTEM") {
          this.sync_inherit = [
            {
              label: `${inheritTr} (${pk_dataset[0].sync.rawvalue})`,
              value: "INHERIT",
            },
          ];
          this.compression_inherit = [
            {
              label: `${inheritTr} (${pk_dataset[0].compression.rawvalue})`,
              value: "INHERIT",
            },
          ];
          this.deduplication_inherit = [
            {
              label: `${inheritTr} (${pk_dataset[0].deduplication.rawvalue})`,
              value: "INHERIT",
            },
          ];
          ...
          this.ws
            .call("pool.dataset.query", [[["id", "=", parent_dataset]]])
            .subscribe((parent_dataset_res) => {
              ...
              if (
                pk_dataset[0].sync.source === "INHERITED" ||
                pk_dataset[0].sync.source === "DEFAULT"
              ) {
                this.sync_collection = [
                  {
                    label: `${inheritTr} (${parent_dataset_res[0].sync.rawvalue})`,
                    value: parent_dataset_res[0].sync.value,
                  },
                ];
              } else {
                this.sync_collection = [
              ...
              if (pk_dataset[0].compression.source === "DEFAULT") {
                this.compression_collection = [
                  {
                    label: `${inheritTr} (${parent_dataset_res[0].compression.rawvalue})`,
                    value: parent_dataset_res[0].compression.value,
                  },
                ];
              } else {
                this.compression_collection = [
              ...
              const compression = _.find(this.fieldConfig, {
                name: "compression",
              });
  ...
  addSubmit(body: any) {
    ...
    if (data.volblocksize !== "INHERIT") {
      let volblocksize_integer_value = data.volblocksize.match(
