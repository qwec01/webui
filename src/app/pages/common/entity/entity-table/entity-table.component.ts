...
export class EntityTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild("defaultMultiActions", { static: false })
  defaultMultiActions: ElementRef;
  ...
  public get currentColumns(): any[] {
    ...
    if (
      ...
      (this.hasDetails() === false || !this.hasDetails)
    ) {
  ...
  constructor(
    ...
    this.core
      .register({ observerClass: this, eventName: "UserPreferencesChanged" })
  ...
  ngOnInit() {
    ...
    this.sortKey =
      this.conf.config.deleteMsg && this.conf.config.deleteMsg.key_props
        ? this.conf.config.deleteMsg.key_props[0]
        : this.conf.columns[0].prop;
    setTimeout(async () => {
      if (this.conf.prerequisite) {
        await this.conf.prerequisite().then((res) => {
          if (res) {
  ...
  handleData(res, skipActions = false): any {
    ...
    if (this.currentRows && this.currentRows.length > 0) {
      ...
      if (!this.conf.noAdd) {
        this.emptyTableConf["message"] +=
          T(` Please click the button below to add `) + this.title + T(`.`);
  ...
  isTableOverflow() {
    ...
    if (this.entitytable) {
      hasHorizontalScrollbar =
        this.entitytable._elementRef.nativeElement.parentNode.scrollWidth >
        this.entitytable._elementRef.nativeElement.parentNode.clientWidth;
    }
  ...
  resetColViewToDefaults() {
    if (
      !(
        this.conf.columns.length === this.originalConfColumns.length &&
        this.conf.columns.length === this.allColumns.length
      )
    ) {
      this.conf.columns = this.originalConfColumns;
  ...
  onHover(evt, over = true) {
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      if (
        ...
        cell.classList.contains("threedot-column")
      ) {
        if (over) {
