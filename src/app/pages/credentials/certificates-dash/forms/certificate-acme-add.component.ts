...
export class CertificateAcmeAddComponent {
  ...
  preInit(entityForm: EntityFormComponent) {
    this.ws.call("acme.dns.authenticator.query").subscribe((authenticators) => {
      this.dns_map = _.find(this.fieldSets[2].config[0].templateListField, {
        name: "authenticators",
      });
    ...
    this.ws.call("certificate.acme_server_choices").subscribe((choices) => {
      const acme_directory_uri = _.find(this.fieldSets[0].config, {
        name: "acme_directory_uri",
      });
  ...
  customSubmit(value) {
    ...
    this.dialogRef = this.dialog.open(EntityJobComponent, {
      data: { title: helptext_system_certificates.acme.job_dialog_title },
      disableClose: true,
    });
