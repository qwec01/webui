...
export class ChartReleasesComponent implements OnInit {
  ...
  update(name: string) {
    this.translate.get(helptext.charts.upgrade_dialog.msg).subscribe((msg) => {
      this.dialogService
        .confirm(helptext.charts.upgrade_dialog.title, msg + name + "?")
        .subscribe((res) => {
          if (res) {
  ...
  doRollback(entityDialog: any) {
    ...
    self.dialogRef = self.mdDialog.open(EntityJobComponent, {
      data: { title: helptext.charts.rollback_dialog.job },
      disableClose: true,
    });
    self.dialogRef.componentInstance.setCall("chart.release.rollback", [
      self.rollbackChartName,
      payload,
    ]);
  ...
  delete(name: string) {
    this.translate.get(helptext.charts.delete_dialog.msg).subscribe((msg) => {
      this.dialogService
        .confirm(helptext.charts.delete_dialog.title, msg + name + "?")
        .subscribe((res) => {
          if (res) {
            this.dialogRef = this.mdDialog.open(EntityJobComponent, {
      ...
      this.dialogService
        ...
        .subscribe((res) => {
          if (res) {
            ...
            this.dialogRef.componentInstance.setCall("core.bulk", [
  ...
  afterShellDialogInit(entityDialog: any) {
    ...
    entityDialog.formGroup.controls["pods"].valueChanges.subscribe((value) => {
      ...
      const containerFC = _.find(entityDialog.fieldConfig, {
        name: "containers",
      });
