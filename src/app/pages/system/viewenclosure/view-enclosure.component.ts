...
export class ViewEnclosureComponent
  ...
  constructor(private core: CoreService, protected router: Router) {
    ...
    core
      .register({ observerClass: this, eventName: "EnclosureData" })
      .subscribe((evt: CoreEvent) => {
    ...
    core
      .register({ observerClass: this, eventName: "EnclosureLabelChanged" })
      .subscribe((evt: CoreEvent) => {
  ...
  extractVisualizations() {
    this.system.profile.forEach((item, index) => {
      if (this.system.rearIndex && item.enclosureKey == this.system.rearIndex) {
        return;
      }
      if (this.system.profile) {
        this.events.next({
          name: "CanvasExtract",
          data: this.system.profile[index],
          sender: this,
        });
      }
    });
