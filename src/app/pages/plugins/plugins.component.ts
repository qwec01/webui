...
export class PluginsComponent {
    ...
    deleteMsg: {
      ...
      doubleConfirm: (item) => {
        return this.dialogService.doubleConfirm(
          ...
          T("Deleting the <b>") +
            item.name +
            T("</b> plugin deletes all data and snapshots stored with it."),
          item.name,
          true
        );
      },
    ...
    },
    {
      ...
      onClick: (selected) => {
        ...
        this.dialogService.Info(
          helptext.multi_update_dialog.title,
          helptext.multi_update_dialog.content
        );
        this.entityList.busy = this.ws
            ...
            (res) => {
              ...
              this.updateRows(selected).then(() => {
                if (message === "") {
                  ...
                  this.dialogService.Info(
                    helptext.multi_update_dialog.title,
                    helptext.multi_update_dialog.succeed
                  );
                } else {
      ...
      if (this.availablePools !== undefined) {
        this.ws
          ...
          .then(
            ...
            (err) => {
              this.dialogService
                .errorReport(err.trace.class, err.reason, err.trace.formatted)
                .subscribe((res) => {
                  resolve(false);
                });
            }
      ...
      fieldConfig: [
        {
          ...
          options: this.availablePools
            ? this.availablePools.map((pool) => {
                return {
                  label:
                    pool.name +
                    (pool.is_decrypted
      ...
      this.ws.call("plugin.query").subscribe(
        (res) => {
          for (const row of rows) {
            ...
            for (const i in row) {
              row[i] =
                (i === "ip4" || i === "ip6") && res[targetIndex][i] != null
                  ? res[targetIndex][i]
                      .split(",")
                      .map(function (e) {
  ...
  updateMultiAction(selected: any) {
    if (
      _.find(selected, function (plugin) {
        return plugin.state == "up";
      })
    ) {
  ...
  isActionVisible(actionId: string, row: any) {
    if (actionId === "start" && row.state === "up") {
      ...
      actionId === "management" &&
      (row.state === "down" || row.admin_portals.length === 0)
    ) {
      return false;
        ...
        fieldConfig: [
          {
            ...
            options: row.admin_portals
              ? row.admin_portals.map((item) => {
