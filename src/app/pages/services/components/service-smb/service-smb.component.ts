...
export class ServiceSMBComponent {
...

    {
      ...
      config: [
        ...
        },
        {
          ...
          alert: {
            message:
              "Higher log levels are for debuging purposes only and should not be set on production servers. Setting a log level too high may significantly impact SMB server performance and mask errors (because the log file will roll over too quickly).",
  ...
  preInit(entityForm: any) {
    ...
    const otherSet = _.find(this.fieldSets, {
      name: helptext.cifs_srv_fieldset_other,
    });
    ...
    this.ws.call("smb.unixcharset_choices").subscribe((res) => {
      ...
      for (let i = 0; i < values.length; i++) {
        this.cifs_srv_unixcharset.options.push({
          label: values[i],
          value: values[i],
        });
      }
