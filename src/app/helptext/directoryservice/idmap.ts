...
export default {
  idmap: {
    ...
    name: {
      ...
      options: [
        {
          label: T("Active Directory - Primary Domain"),
          value: "DS_TYPE_ACTIVEDIRECTORY",
        },
    ...
    range_tooltip: T(
      "Range Low and Range High set the range of UID/GID numbers which this \
 ...
 number outside this range, the external credential is ignored."
    ...
    idmap_backend: {
      ...
      tooltip: T(
        "Provides a plugin interface for Winbind to use varying backends to store \
 SID/uid/gid mapping tables. The correct setting depends on the environment in which the NAS is deployed."
    ...
    certificate_id: {
      ...
      tooltip: T(
        "Select the certificate of the Active Directory server\
 ...
 System/Certificates menu."
    ...
    schema_mode: {
      ...
      tooltip: T(
        'Choose the schema to use with LDAP authentication for\
 ...
 For SFU 3.0 or 3.5, choose "SFU". For SFU 2.0, choose "SFU20".'
    ...
    unix_primary_group: {
      ...
      tooltip: T(
        'When checked, the primary group membership is fetched from the LDAP \
 ...
 the "primaryGroupID" LDAP attribute.'
    ...
    unix_nss: {
      ...
      tooltip: T(
        "When checked, winbind will retrieve the login shell and home directory \
 ...
 the smb4.conf parameters <code>template shell</code> and <code>template homedir</code> are used."
    ...
    rangesize: {
      ...
      tooltip: T(
        "Define the number of UIDS/GIDS available per domain \
 range. The minimum is <i>2000</i> and the recommended default is <i>100000</i>."
    ...
    readonly: {
      ...
      tooltip: T(
        "Set to make the module <i>read-only</i>. No new ranges \
 ...
 domain."),
    ...
    ldap_basedn: {
      ...
      tooltip: T(
        'The directory base suffix to use for SID/uid/gid\
 ...
 target="_blank">smb.conf</a>.'
    ...
    ldap_url: {
      placeholder: T("URL"),
      tooltip: T(
        "LDAP server to use for SID/uid/gid map entries. When\
 ...
 Example: <i>ldap://ldap.netscape.com/o=Airius.com</i>."
    ...
    ldap_server: {
      ...
      tooltip: T(
        "Select the type of LDAP server to use. This can be the\
    ...
    cn_realm: {
      ...
      tooltip: T(
        "Append <i>@realm</i> to <i>cn</i> in LDAP queries for\
 both groups and users when User CN is set)."
    ...
    ldap_domain: {
      ...
      tooltip: T(
        "The domain to access the Active Directory server when\
 using the LDAP server inside the Active Directory server."
    ...
    clear_cache_dialog: {
      ...
      message: T(
        'The Idmap cache should be cleared after finalizing idmap changes. \
