...
export class TwoFactorComponent {
  ...
  public custActions: Array<any> = [
    {
      ...
      function: () => {
        this.dialog
          .confirm(
            helptext.two_factor.confirm_dialog.title,
            helptext.two_factor.confirm_dialog.message,
            true,
            helptext.two_factor.confirm_dialog.btn
          )
          .subscribe((res) => {
            if (res) {
              this.loader.open();
  ...
  getURI() {
    this.ws.call("auth.twofactor.provisioning_uri").subscribe(
      (res) => {
  ...
  customSubmit(data) {
    if (data.otp_digits === this.digitsOnLoad && data.interval === this.intervalOnLoad) {
      ...
      this.dialog
        .confirm(
          helptext.two_factor.submitDialog.title,
          helptext.two_factor.submitDialog.message,
          true,
          helptext.two_factor.submitDialog.btn
        )
        .subscribe((res) => {
          if (res) {
            this.intervalOnLoad = data.interval;
  ...
  renewSecret() {
    this.dialog
      .confirm(
        helptext.two_factor.renewSecret.title,
        helptext.two_factor.renewSecret.message,
        true,
        helptext.two_factor.renewSecret.btn
      )
      .subscribe((res) => {
        if (res) {
          this.loader.open();
  ...
  updateSecretAndUri() {
    this.ws.call("auth.twofactor.config").subscribe(
      (res) => {
