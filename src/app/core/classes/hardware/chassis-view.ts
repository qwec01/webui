...
export class ChassisView {
  ...
  requiredAssets() {
    ...
    assets.push({
      alias: this.model + "_drivetray_bg",
      path: this.driveTrayBackgroundPath,
    });
    assets.push({
      alias: this.model + "_drivetray_handle",
      path: this.driveTrayHandlePath,
    });
    return assets;
  }
  ...
  load() {
    ...
    if (this.altDriveTraySlots.length > 0) {
      ...
      this.loader
        .add(this.model + "_alt_drivetray_bg", this.altDriveTrayBackgroundPath)
    ...
    for (let i = 0; i < this.totalDriveTrays; i++) {
      ...
      if (this.autoPosition) {
        let position = this.generatePosition(
          dt.container,
          i,
          this.driveTraysOffsetX,
          this.driveTraysOffsetY
        );
