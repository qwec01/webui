...
export class LdapComponent {
  ...
  afterInit(entityEdit: any) {
    ...
    this.ws.call("kerberos.realm.query").subscribe((res) => {
      this.ldap_kerberos_realm = _.find(this.fieldConfig, {
        name: "kerberos_realm",
      });
      res.forEach((item) => {
    ...
    this.ws.call("ldap.ssl_choices").subscribe((res) => {
      ...
      res.forEach((item) => {
        this.ldap_ssl.options.push({ label: item, value: item });
      });
    });
    ...
    this.systemGeneralService.getCertificates().subscribe((res) => {
      ...
      res.forEach((item) => {
        this.ldapCertificate.options.push({ label: item.name, value: item.id });
      });
    ...
    this.ws.call("ldap.schema_choices").subscribe((res) => {
      ...
      res.forEach((item) => {
        this.ldap_schema.options.push({ label: item, value: item });
      });
    });
    ...
    entityEdit.formGroup.controls["enable"].valueChanges.subscribe((res) => {
      ...
      if (!res) {
        this.entityForm.formGroup.controls["hostname_noreq"].setValue(
          this.entityForm.formGroup.controls["hostname"].value
        );
      } else {
        this.entityForm.formGroup.controls["hostname"].setValue(
          this.entityForm.formGroup.controls["hostname_noreq"].value
        );
      }
    });
