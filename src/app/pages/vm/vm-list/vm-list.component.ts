...
@Component({
  selector: "vm-list",
  template: ` <div class="vm-summary">
      <p *ngIf="availMem">
        <strong>{{ memTitle | translate }}</strong> {{ availMem }} -
        {{ memWarning | translate }}
      </p>
    </div>
...
export class VMListComponent implements OnDestroy {
  ...
  refreshVMWizard() {
    this.addComponent = new VMWizardComponent(
      this.rest,
      this.ws,
      this.vmService,
      this.networkService,
  ...
  resourceTransformIncomingRestData(vms) {
    for (let vm_index = 0; vm_index < vms.length; vm_index++) {
      ...
      vms[vm_index]["memory"] = this.storageService.convertBytestoHumanReadable(
        vms[vm_index]["memory"] * 1048576,
        2
      );
    }
    return vms;
  ...
  checkDisplay(vm) {
    ...
    if (
      ...
      (vm.bootloader === "GRUB" || vm.bootloader === "UEFI_CSM")
    ) {
      return false;
    }
        ...
        fieldConfig: [
          {
            ...
            tooltip: T(
              "Force the VM to stop if it has not already \
  ...
  doRowAction(row, method, params = [row.id], updateTable = false) {
    if (method === "vm.stop") {
      this.dialogRef = this.dialog.open(EntityJobComponent, {
        data: { title: T("Stopping " + row.name) },
        disableClose: false,
      });
 ...
 After Timeout</i> option to stop the VM."
              )
          ),
          "450px",
          "info",
      ...
      this.ws.call(this.queryCall).subscribe(
        (res) => {
          for (const row of rows) {
            const targetIndex = _.findIndex(res, function (o) {
              return o["id"] === row.id;
            });
            if (targetIndex === -1) {
              reject(false);
            ...
            fieldConfig: [
              {
                ...
                value: false,
                tooltip: T(
                  "Set to remove the data associated with this \
 ...
 the Virtual Machine when it is still active or has an undefined state."
      ...
      },
      {
        ...
        onClick: (display_vm) => {
          this.loader.open();
          this.ws.call("vm.get_display_devices", [display_vm.id]).subscribe(
            (res) => {
              this.loader.close();
              const conf: DialogFormConfiguration = {
                ...
                fieldConfig: [
                  {
                    ...
                    options: res.map((d) => {
                      return { label: d.attributes.type, value: d.id };
                    }),
                ...
                customSubmit: (entityDialog) => {
                  const display_device = _.find(res, {
                    id: entityDialog.formValue.display_device,
                  });
                  if (display_device.attributes.password_configured) {
                    const pass_conf: DialogFormConfiguration = {
                      ...
                      customSubmit: (passDialog) => {
                        this.loader.open();
                        this.ws
                          .call("vm.get_display_web_uri", [
                            display_device.id,
                            passDialog.formValue.password,
                          ])
                          .subscribe(
                            (res) => {
                              this.loader.close();
  ...
  isActionVisible(actionId: string, row: any) {
    if (
      actionId === "DISPLAY" &&
      (row["status"]["state"] !== "RUNNING" || !this.checkDisplay(row))
    ) {
      return false;
    } else if (
      (actionId === "POWER_OFF" ||
