...
export class DisplayObject {
  ...
  set moveable(value: boolean) {
    if (value) {
      ...
      listen(this.moveHandle, "mousedown touchstart")
        .while((v) => this._moveable)
  ...
  set resizeable(value: boolean) {
    if (value) {
      ...
      listen(this.resizeHandleTop, "mousedown touchstart")
        .while((v) => this._resizeable)
        .start(this.startResizeTop.bind(this));
      listen(this.resizeHandleBottom, "mousedown touchstart")
        .while((v) => this._resizeable)
        .start(this.startResizeBottom.bind(this));
      listen(this.resizeHandleLeft, "mousedown touchstart")
        .while((v) => this._resizeable)
        .start(this.startResizeLeft.bind(this));
      listen(this.resizeHandleRight, "mousedown touchstart")
        .while((v) => this._resizeable)
        .start(this.startResizeRight.bind(this));

      listen(document, "mouseup touchend")
        .while((v) => this._moveable)
        .start(this.stopResize.bind(this));
  ...
  start() {
    ...
    this.pointerTracker = pointer(this.anchorXY.get())
      .pipe(
        stream,
        transformMap({
          y: (v) => {
      ...
      .start(this.anchorXY);
    if (this.anchored) {
      this.focus = pointer(this.anchorXY.get()).start(this.unfocus);
    }
    ...
    if (this.constrainX || this.constrainY) {
      listen(document, "mouseup touchend")
        .while((v) => this._moveable)
        .start(this.stopDrag.bind(this));
    } else {
      listen(this.rawElement, "mouseup touchend")
        .while((v) => this._moveable)
        .start(this.stopDrag.bind(this));
      listen(document, "mouseleave")
        .while((v) => this._moveable)
        .start(this.mouseExit.bind(this));
    }
  }
