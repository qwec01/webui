...
export class NetworkComponent extends ViewControllerComponent implements OnInit, OnDestroy {
  ...
  public interfaceTableConf = {
    ...
    afterGetData: function (res) {
      const state = this.parent.navigation.extras.state as {
  ...
  getGlobalSettings() {
    this.ws.call(this.configCall).subscribe((config_res) => {
      this.ws.call(this.summayCall).subscribe((res) => {
        ...
        this.globalSettingsWidget.data.netwait = config_res.netwait_enabled
          ? T("ENABLED")
          : T("DISABLED");
        let tempArr = [];
        ...
        this.globalSettingsWidget.data.httpproxy =
          config_res.httpproxy !== "" ? config_res.httpproxy : "---";
        this.globalSettingsWidget.data.hostnameDB =
          config_res.hosts !== "" ? config_res.hosts : "---";
        ...
        if (config_res.activity.type === "DENY") {
          ...
          this.globalSettingsWidget.data.outbound =
            T("Allow ") + config_res.activity.activities.join(", ");
        }
      });
    });
          ...
          this.ws.call("service.stop", [rowinner.service]).subscribe(
            ...
            (err) => {
              ...
              this.dialog.errorReport(
                T("Error stopping service OpenVPN ") + rowinner.service_label,
                err.message,
                err.stack
              );
            }
        ...
        onClick: (rowinner) => {
          ...
          this.ws.call("service.start", [rowinner.service]).subscribe(
            ...
            (err) => {
              rowinner["onChanging"] = false;
              this.dialog.errorReport(
                T("Error starting service OpenVPN ") + rowinner.service_label,
                err.message,
                err.stack
              );
            }
  ...
  isOpenVpnActionVisible(name, row) {
    if (
      (name === "start" && row.state === "RUNNING") ||
      (name === "stop" && row.state === "STOPPED")
    ) {
      return false;
    }
    return true;
